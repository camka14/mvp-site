...
- [x] (2026-07-20 18:24 PDT) Installed and mode-checked the live Stripe secret, publishable, webhook, and Connect credentials plus Apple key `6H4679F5X8`; OpenSSL validated the Apple private key before deployment.
- [ ] Complete the production acceptance suite (completed: health, landing page, filtered Discover Markdown, event and organization HTML/Markdown, `llms.txt`, `/api/app-version`, original and resized Spaces files, and expected unauthenticated rejection through both WebSocket upgrade paths; remaining: authenticated login, payment callbacks, and authenticated WebSocket sessions).
- [x] (2026-07-20 18:48 PDT) Completed apex and `www` traffic cutover: detached only the custom web domains from App Platform, moved authoritative DNS to `15.204.81.193`, issued both certificates, opened production traffic, confirmed apex HTTP 200, and confirmed permanent `www` redirect to the apex.
...
- [x] (2026-07-20 18:38 PDT) Completed the write-frozen database cutover after the live-secret gate passed: final encrypted snapshot `e0322fe1`, exact counts for all 95 public tables, 162 applied migrations, local runtime-role grants, healthy app on local PostgreSQL, public writes admitted only after DNS, and the first encrypted post-cutover local backup completed at `2026-07-21T01:38:03Z`.
...
- Observation: App Platform-owned DNS records cannot be edited in place.
  Evidence: DigitalOcean rejected the first apex update with HTTP 422 because the record was managed by the app. Removing only `bracket-iq.com` and `www.bracket-iq.com` from the maintained App Platform spec released the managed A, AAAA, and CNAME records; new apex A and `www` CNAME records could then be created without touching mail, verification, or preview records.

- Observation: certificate issuance can briefly see removed App Platform addresses after authoritative DNS is correct.
  Evidence: the former apex A and AAAA records had 30-second TTLs and the former `www` CNAME had a 1,800-second TTL. Caddy's first ACME attempt reached cached Cloudflare addresses, then issued the apex certificate on retry after the authoritative zone and public resolver both returned only `15.204.81.193`. A later retry issued the `www` certificate after Google and Cloudflare resolvers both returned the new CNAME target.

- Observation: valid Spaces credentials are insufficient when `STORAGE_PROVIDER` explicitly selects local storage.
  Evidence: the first public file checks returned 404 even though the restored `File` rows, bucket, endpoint, key, and secret were present. The VPS `app.env` still contained `STORAGE_PROVIDER='local'`; changing only that value to `spaces` and recreating the app made an original 1,662,343-byte PNG, its 128-pixel preview, and an affiliate organization logo return HTTP 200 through the VPS.
...
- Decision: Keep the old App Platform app and managed database undeleted and in maintenance after local PostgreSQL becomes authoritative.
  Rationale: the old services are no longer allowed to accept writes, but retaining them through the observation window preserves rollback evidence and avoids an irreversible decommission during cutover. Any rollback after public local writes would require a deliberate forward data reconciliation rather than simply re-enabling the stale managed path.
  Date/Author: 2026-07-20 / Codex

## Outcomes & Retrospective

Repository, host, and cutover outcome as of 2026-07-20: immutable image `dc32c2d314bd2e01c3ba87aab65a5be4812c43a9` runs as UID 1001 on the OVHcloud VPS with healthy Caddy, Next.js, PostgreSQL 17, and Redis containers. The final managed source measured 40,621,747 bytes; its 2,278,258-byte custom dump covered 95 public tables and 162 migrations, took 26 seconds to produce, two seconds to upload encrypted, and four seconds to restore and validate. Snapshot `e0322fe1` and the first post-cutover local snapshot are stored in the client-side encrypted Spaces Restic repository. Apex and `www` DNS resolve to `15.204.81.193`, both certificates are active, apex returns 200, and `www` permanently redirects to the apex. Health, landing, Discover, event/org HTML and Markdown, `llms.txt`, affiliate-destination omission, app-version, Spaces file/preview, and WebSocket upgrade-routing checks pass from the VPS. The old App Platform app and managed database are retained but maintained and no longer own the production domains; local PostgreSQL is authoritative. Remaining observation work is authenticated and callback acceptance, ongoing backup checks, and eventual explicitly approved decommission.
