...
- [ ] Build the reviewed training dataset, run the first parameter-efficient adapter experiment, and promote an adapter only if it beats the frozen base model. (2026-07-29 20:24Z: added the immutable SFT-release builder and shared inference/training prompt; it accepts only human-approved, evidence-bound `FAITHFUL` or `BLOCKED` teaching envelopes and rejects secrets, stale/partial rows, unsafe refusals, and domain split leakage. No current historical row passes the gold-example gate, and no training job has run.)
