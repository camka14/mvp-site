...
- [ ] Deploy revised cohort `affiliate-mapping-test-aa6aa626e3f2367c` to the live OVH application container, run the Sherwood intake through ScrapingDog, export the selected live run, and rerun the complete 35-source audit. The owner supplied corrected OVH address `15.204.80.66`, but it presents a different SSH host key and rejects every existing local production key for the expected operator users. Meanwhile authoritative public DNS and live HTTPS still resolve `bracket-iq.com` to `15.204.81.193`; that host serves the healthy app but refuses port 22, while direct TLS to the corrected address does not serve the BracketIQ certificate. Confirm which VM owns the authoritative app/database and provide its current SSH user/key before writing. Do not substitute the retired database or mark the cohort complete.
