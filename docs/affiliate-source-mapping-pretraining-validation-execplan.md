...
- [ ] Resolve the Sherwood locked-URL mismatch before freezing all 35 real test examples. The live source and its working canonical site use `https://www.sherwoodsoccer.org/`, but the approved proposal requires `https://sherwoodsoccer.org/`, whose certificate is still expired from the OVH runtime. Either repair the apex certificate and capture the exact locked URL, or create and explicitly approve a new cohort version whose Sherwood required page is the canonical `www` URL. The other capture issues and older-provider reuses are resolved; Team Lillard remains the approved blocked-source refusal.
