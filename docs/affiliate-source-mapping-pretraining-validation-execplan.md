...
- [ ] Deploy revised cohort `affiliate-mapping-test-aa6aa626e3f2367c` to the live OVH application container, run the Sherwood intake through ScrapingDog, export the selected live run, and rerun the complete 35-source audit. The public live site remains HTTP 200, but OVH port 22 began refusing connections after the revision was locked and the available in-app admin session is unauthenticated; do not substitute the retired database or mark the cohort complete until live access is restored.
