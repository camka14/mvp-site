...
| **Completed — high, continued (1)** | `TEST-011` | `ebf50ab5` adds canonical-only active player/staff Schedule coverage. The current money-display and participant-operation regressions were rerun with it, covering cent/display conversion, one-shot mutation guards, and stale A/B response rejection. |
| **Completed — other severity, continued (1)** | `APP-129` | `ebf50ab5` makes Schedule use synchronized canonical active membership with legacy arrays confined to the compatibility normalizer. Its four focused regressions passed. |
| **Remaining / not yet reconciled (81)** | All other headings in this report | Do not infer completion from an old or partial implementation. Each item must receive a current-source review, a focused regression test where code changes, and browser/emulator evidence when reachable. |

Current strict count: **146 completed, 81 remaining or not yet reconciled, 227 total findings**. This count deliberately excludes any pre-existing change that has not yet been revalidated against the current audit scenario.
