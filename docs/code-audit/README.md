...
- Fix status: **decomposition in progress on the audited branch**. `EventDetailSheet.tsx` is now 1,127 lines and imports no event, payment, or registration service, while `EventForm.tsx` is now 1,479 lines with React Hook Form retained as its sole persisted draft owner and no direct effects. Focused controllers own stale request rejection, registration phase exclusivity, signing, checkout, navigation, presentation state, resources, slots, division editing/commit/normalization, staffing, submission, create-form reference hydration, cross-field invariants, atomic configuration actions, and catalog/context hydration; render-only views own the public details, registration panels, join card, event hero, responsive sheet/inline content layout, and manual-payment settings. All new interaction owners remain below 600 lines and their direct regressions pass, but the approximately 600/700-line facade targets plus production-build browser acceptance have not yet been reached, so AUD-003 remains open.
