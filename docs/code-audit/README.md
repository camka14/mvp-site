...
Status: **complete — remediation tracked below**
...
## Mobile event data-propagation runtime audit — remediated batch
...
| --- | --- | --- | --- | --- |
| `MOB-RUNTIME-001` | Create multiple event types in Android and verify image, divisions, fields, slots, resources, officials, and rules survive API → DB → cold Room reload → edit. | Fixed | Partial runtime pass | `a8581c2f` limits mandatory fields to League and Tournament while preserving optional fields on regular events. EVENT and WEEKLY_EVENT fieldless acceptance plus LEAGUE and TOURNAMENT rejection pass in the focused repository suite. `69f44a6a` also makes backend capacity/save errors visible after loading closes. The prior league persistence evidence remains valid; full multi-type edit round-trip coverage remains broader than this repair batch. |
| `MOB-RUNTIME-002` | Remove current reliance on event `teamIds`/`userIds`; use `EventRegistrations`, participant snapshots, and Room relations. | Compatibility follow-up | Current symptom disproven | Active compatibility arrays still need migration, but the audited missing-Schedule symptom no longer reproduces. The canonical route tests pass, and the installed app displayed `Manual Check-in Smoke 115526` with seven matches without adding a legacy fallback. |
| `MOB-RUNTIME-003` | Clear app data, sign in as a second user, discover and join created events, then see them and their matches in Schedule. | Canonical path verified | Pass | `player@example.com` sees the team-registered `Manual Check-in Smoke 115526` event with seven matches on a cold authenticated Schedule load. Leaving and returning triggers fresh `/api/profile/schedule` requests and repopulates the same result. |
| `MOB-RUNTIME-004` | Score a match and verify canonical segment state propagates live and survives cold reload. | Fixed | Regression/build verified | `69f44a6a` retains the confirmed optimistic state until the repository snapshot semantically contains the completed segment, then accepts later canonical score updates. The regression includes a server-enriched segment timestamp so canonical enrichment does not keep the overlay stuck. |
| `MOB-RUNTIME-005` | Confirm a set and verify the narrow operation succeeds without sending cached policy snapshots or resetting prior sets. | Fixed | Backend regression verified | `a8581c2f` normalizes a committed sibling match with a null/invalid end using the configured match duration and retains the inferred conflict window, preventing both the crash and official double-booking. |
| `MOB-RUNTIME-006` | Trigger event team check-in by moving the event window backward; verify the registered/managed team is recognized after cold cache. | Issue confirmed | Pass with compatibility risk | After a cold restart, the EVENT-mode prompt offered `Smoke Team Alpha` and persisted an EVENT `TeamCheckIns` row. Eligibility still depends on the derived `Event.teamIds` projection instead of canonical registration relations, so omission or staleness of that projection can suppress the prompt. |
| `MOB-RUNTIME-007` | Trigger match official check-in by moving match time backward; verify assignment, check-in state, and reopen behavior. | Issue confirmed | Pass with compatibility risk | With a canonical `EventOfficials` assignment, the MATCH-mode prompt appeared, persisted a MATCH `TeamCheckIns` row, and opened the roster dialog. A legacy-only official assignment rendered an official prompt but the server correctly rejected it with 403, demonstrating that client fallback state can disagree with canonical authorization. |
| `MOB-RUNTIME-008` | Verify invitations, participant management, current-user registration cache, logout, and account switch do not leak or drop state. | Static-reviewed | Partial pass | Application data was cleared and the second account was authenticated from a cold state; its individual registration and Schedule membership rehydrated correctly. No cross-account leakage was observed. Invitation delivery was not independently exercised in this runtime pass. |
| `MOB-RUNTIME-009` | Remove current legacy event/match parameters after all consumers migrate; retain only explicit historical DB migration code. | Issue confirmed | Static evidence complete | Active compatibility remains in event participant arrays, event-team check-in selection, registration mutation fallback, single-official fields, legacy score arrays, and team player/pending arrays. Historical Room migrations are legitimate upgrade-only code; the active fallbacks are not yet removable because current consumers still depend on them. |

Runtime remediation summary on 2026-07-16: the seven-item defect review closed with **6 fixes and 1 current-state reclassification**. Backend `a8581c2f` repairs event field validation and staffing resilience. Mobile `69f44a6a` repairs team-registration controls, league defaults, visible create errors, and post-confirm score freshness. Canonical Schedule loading and return refresh were verified without a legacy fallback. Focused backend coverage passed 77 tests plus TypeScript checking; Android main/unit-test sources compiled, the debug APK installed, and UI-tree evidence is under `mvp-app/artifacts/mobile-runtime-audit/`.

The emulator produced two application-not-responding dialogs while the Mac was under sustained load. Both are classified as environment-inconclusive rather than product defects because the surrounding flows completed on retry and the user reported that the computer was running slowly.
...
| **Completed — high (89)** | `SEC-002`, `SEC-003`, `SEC-004`, `SEC-005`, `SEC-006`, `SEC-007`, `SEC-008`, `SEC-010`, `SEC-013`, `SEC-016`, `SEC-019`, `SEC-021`, `SEC-022`, `SEC-024`, `SEC-031`, `SEC-032`, `SEC-033`, `SEC-034`, `SEC-035`, `SEC-036`, `SEC-039`, `SEC-041`, `SEC-042`, `TEST-004`, `TEST-005`, `TEST-006`, `OPS-004`, `DB-002`, `DB-003`, `DATA-014`, `DATA-015`, `DATA-016`, `DATA-019`, `DATA-020`, `DATA-022`, `DATA-023`, `DATA-028`, `DATA-030`, `DATA-001`, `DATA-002`, `DATA-003`, `DATA-004`, `DATA-005`, `DATA-006`, `DATA-010`, `APP-002`, `APP-004`, `APP-014`, `APP-017`, `APP-026`, `APP-027`, `APP-028`, `APP-029`, `APP-044`, `APP-052`, `APP-055`, `APP-061`, `APP-063`, `APP-065`, `APP-066`, `APP-069`, `APP-074`, `APP-077`, `APP-079`, `APP-080`, `APP-081`, `APP-083`, `APP-084`, `APP-088`, `APP-089`, `APP-090`, `APP-095`, `APP-098`, `APP-101`, `APP-103`, `APP-105`, `APP-109`, `APP-110`, `APP-116`, `APP-117`, `APP-121`, `APP-124`, `APP-130`, `APP-131`, `APP-132`, `APP-133`, `APP-134`, `APP-135`, `UI-001` | `a5ce0257`, `1731ad84`, `36e1afd6`, `3ed10d0f`, `1aaafb77`, `aafb360f`, `696bf484`, `58466c56`, `79db2c13`, `b3826149`, `d5d6592e`, `2401d0ee`, `eaca7349`, `bb1b1177`, `531ab0b5`, `68abaf75`, `7ee85fd8`, `5b06fffe`, `a00ff409`, `a2ba3569`, `de36f944`, `86611865`, `28550b5c`, `ca620ba4`, `f64bf8e2`, `2fd43b7b`, `2073720f`, `c83b68c8`, `6786ca86`, `46c9c9c1`, `a095e272`, `d2b7cea8`, `38dc5780`, `5a47758f`, `bc4e2bf8`, `6cc4f89a`, `3cca9a9d`, `3ce1ffac`, `caab4a9c`, `81bff7fa`, `4ca35a13`, `c3be4fc1`, `99d84287`, `4b271007`, `c5de7fbb`, `7ccf037a`, `c38269b0`, `de9bf54d`, `c03e8a94`, `9c294b38`, `4b654c55`, `f2733976`, `04fbfc86`, `4f145d00`, `86f32971`, `a5872288`, `a8139fdf`, `ef180b3e`, `7ea8e5da`, `f0b5d826`, `0c4966c7`, `d0b50ac0`, `0c18e287`, `7a69e204`, `ac03fcb9`, `2fa82c2f`, `552427ff`, `21f5d85f`, `9aa056d1`, `8280ffd0`, `728e63c3`, `a9267179`, `ab279fd9`, `d6599868`, `f53356c9`, `dacf3f37`, `c55f06be`, `143346de`, `732d67cc`, `d776877f`, `24cd188c`, `e98f676a`, `537245bf`, `8a045707`, `ba4306c9`, `6ff66cfe`, and `bc575940`. Focused web suites passed 58 DOB route/validation tests, along with 58 security/data and 26 guest-entry tests and `npx tsc --noEmit`; focused Android suites passed 4 invitation-invalidation, 12 logout/retry, 9 Room migration/DAO tests, 8 payment-failure tests, 4 player-action tests, 2 controlled-search tests, 41 event-creation tests, the 56-test match-content suite, the 5-test rental-resource suite, the 6-test chat terms/send/lifecycle suite, the 4-test chat-list feedback suite, the 5-test match-team label suite, the 1-test roster-dialog suite, the 4-test DOB picker-policy suite, the 19-test Wear data/action/storage/reconciliation suite, the 3-test team-selection suite, the 1-test team-error-feedback UI suite, the 3-test money-display suite, the 10-test participant-management helper suite, the startup-laziness regression, the 2-test multi-day rental-slot suite, the 9-test profile-draft/password-validation suite, the 6-test rental-availability suite, and the focused 2-test request-generation, review-save suite and the 5-test image-coordinator, 7-test picker-feedback, and 42-test create-component suites. All 37 tracked Room schema snapshots are versioned; the on-device Room suite verified all supported historical sources through the current schema. Browser validation reached the guest onboarding choices without a redirect loop; the installed Android app launched without a migration failure or startup ANR. The focused DATA-016 ID-only-navigation batch passed 73 Android tests with zero failures/errors; its debug APK reached the login UI and a clean force-stop/relaunch without an app crash, while authenticated routes were unavailable because the supplied account was rejected by the backend. The focused `bash scripts/tests/dev-launcher-contract.sh` regression passed, verifying that `dev.ps1` checks Gradle, adb installation, adb launch, and backend timeout failures before continuing; no PowerShell runtime is installed in this macOS environment for a native Windows execution check. The focused `MapComponentCleanupTest` passed both destruction paths, proving collectors are cancelled and the registered back callback is removed even when location shutdown throws. The focused Team Management lifecycle suite passed both destruction paths, proving its component scope is canceled and its editor callback is disabled and unregistered. The focused `MessageRepositoryHttpTest` passed all three cases, including acknowledged and rejected read receipts, proving Room is updated only after the server accepts the receipt. The focused 15-test `MatchRepositoryHttpTest` suite confirms completed segment scores remain optimistically visible without inventing a match winner for either first-set or plurality-of-segment outcomes. The shared local match applier now backs the match-detail fake, and the focused 57-test component plus 15-test repository batch proves finalization and incident operations use production-equivalent local semantics. Failed incident create/delete enqueues leave the cached incident and score authoritative, while successful scoring incidents update the outbox-backed local projection without an unchecked follow-up save. The APP-066/TEST-006 batch passed 25 focused Android tests, the web chat-history route passed all 6 focused tests, and `npx tsc --noEmit` passed. The APP-069/APP-077/APP-088 batch passed 10 focused web suites (58 tests), `npx tsc --noEmit`, Prisma validation/generation/checked-client verification, a clean 152-migration PostgreSQL replay, and a concurrent canonical-pair upsert that left one row. Android passed the 26-test consolidated Compose batch plus the 1-test availability DTO and 2-test repository batches; `assembleDebug` succeeded, and the freshly installed APK cold-launched to Login with Room open and no crash or ANR. Local browser validation rendered the landing and login surfaces normally; authenticated chat and rental interaction were unreachable because the retained browser session was signed out. The current high-severity batch is committed in `076a0e56`, `54d3aced`, `154ad89e`, `35e05f55`, `0f2c5e42`, `9424946e`, `1c6badfa`, and `fb9702ac`; the focused site rental route passed 15/15 tests plus `npx tsc --noEmit`, and the combined mobile rental batch passed 39/39 tests plus the iOS simulator compile. |
| **Completed — other severity (21)** | `SEC-025`, `SEC-037`, `OPS-002`, `OPS-003`, `DATA-013`, `APP-003`, `APP-008`, `APP-015`, `APP-085`, `APP-086`, `APP-087`, `APP-093`, `APP-094`, `APP-096`, `APP-104`, `APP-106`, `APP-107`, `APP-115`, `APP-118`, `APP-125`, `APP-126` | `c7e57d13` closes canonical notification origins with 84/84 focused tests; `36e1afd6`, `531ab0b5`, and `570c09e9` close the bounded preview, verified-session, and PostHog initialization findings; their focused web reconciliation batch passed 65/65 tests. Current dependency declarations and generated client were revalidated at Prisma 7.8.0; `c38269b0` makes that version alignment an explicit build preflight. `af6d73f2` separates chat success/warning feedback from actionable errors, `24c34e3b` preserves the reader's chat position for unseen incoming messages, `1769abdc` renders explicit chat loading/unavailable states rather than a blank interactive composer, and `b77f6da1` aligns client-side Profile Details username validation with the server's trim/nonblank contract. `95a98de8` retries one transient match check-in read without duplicate startup traffic. `24faca6a` makes event-name validation authoritative and `752082f3` preserves custom match-incident definitions, both with focused regressions. `4c6b7dfc` caps Discover filters to their real positive viewport space and applies date-interval overlap to weekly events; the focused 3-test height and 10-test event-filter suites passed. `59b6d7a0` replaces Profile Details' legacy picker callbacks with a result-state flow that distinguishes user cancellation, empty selection, provider failure, conversion failure, and upload failure; each non-cancel failure supplies a Try again action. The focused 6-test picker-feedback and 9-test Profile Details draft suites passed. `cd528842` removes the obsolete subscription fallback so supported-endpoint 404s and transport failures stay visible while a true empty 200 remains valid; its focused three-case HTTP suite passed. `2928a31c` returns a successful empty field response as the authoritative list instead of resurrecting cached facilities; its focused HTTP regression passed. `7b6ef71f` cancels superseded connection requests and accepts a search or refresh result only when its request identity still matches the current state; the focused 4-test tracker suite passed. `ac3862e3` holds creation-form validation feedback until interaction or an invalid submit attempt; its focused three-test Compose UI suite passed. `fb9702ac` completes overnight and early-morning rental scheduling, while `54d3aced`, `154ad89e`, and `9424946e` move Root's center action onto the bounded next-action contract; the focused rental and schedule validations passed on current source. |
| **Completed — high, continued (4)** | `TEST-008`, `TEST-011`, `APP-092`, `APP-123` | `fa121d77` adds generation-guarded invite searches and clear/dismiss invalidation; its focused 12-test coordinator suite passed, while the already-completed rental-resource suite covers selection-on and selection-off draft synchronization. `0720f586` and `c40797cd` carry exact per-selection rental windows through canonical pricing, temporary holds, and final order creation. The four focused web suites passed 56/56 tests plus `npx tsc --noEmit`; mobile passed 59/59 focused repository/component tests and the iOS simulator shared compile. The installed Android branch cold-launched without a fatal exception, ANR, or OOM; authenticated rental UI remained unavailable without a retained session. `ebf50ab5` adds canonical-only active player/staff Schedule coverage and current participant-operation regressions. |
| **Completed — other severity, continued (2)** | `APP-033`, `APP-129` | `ebb342c6` gives Wear email/password inputs persistent distinct accessibility labels and explicit password semantics. Its Compose semantics regression passed; constrained installed-APK validation confirmed both labels persist after entry and the password remains masked. `ebf50ab5` makes Schedule use synchronized canonical active membership with legacy arrays confined to the compatibility normalizer; its four focused regressions passed. |
| **Completed — high, latest batch (1)** | `DATA-017` | Site `9bd0a7d2` adds the versioned authoritative quote endpoint and mobile `69db9bf2` removes local fee policy from every active price editor. The route/canonical fee batch passed 19 tests; the current mobile batch passed 148 tests, TypeScript and iOS compilation passed, and a freshly installed Android build reached Login and the email-signup form without a fatal exception, ANR, or OOM. Authenticated price editors were not reachable without a retained session. |
| **Completed — other severity, latest batch (8)** | `SEC-040`, `TEST-001`, `TEST-002`, `TEST-003`, `TEST-009`, `TEST-010`, `APP-012`, `APP-020` | Site `27f08f00` adds route-inclusive coverage gates and CI. Mobile `bfc6e542`, `4ce92ab7`, and `fb209fc4` close account-scoped registration retention, production/test AndroidX separation, and the Gradle daemon JDK contract; prior focused commits close the Wear, Maps-placeholder, blank-name, and custom-incident test gaps. The clean site coverage run passed 445 suites/2,834 tests; the consolidated mobile batch passed 148 tests, six focused Wear tests, iOS compilation, KSP, installation, and cold-launch checks. |
| **Completed — other severity, follow-up batch (4)** | `SEC-026`, `APP-016`, `APP-059`, `APP-070` | Site `416b6343` constrains remote affiliate-logo downloads across DNS, redirects, time, content type, streamed size, and decoded pixels. Mobile `eacd608d`, `caec47c1`, and `d4a4e475` add an explicit accessible map-pin chooser, generation-safe billing-address resolution, and cycle-safe bracket candidates. The site security suite passed 28 tests plus TypeScript and targeted lint; the combined mobile batch passed 14 tests plus iOS compilation. The exact APK installed and completed Login → email signup → Login without a fatal exception, ANR, or OOM; authenticated target screens were unavailable on the signed-out install. |
| **Completed — other severity, portable launcher/refund batch (2)** | `APP-022`, `APP-075` | Mobile `49295c3c` disables `mavenLocal()` unless explicitly opted in, removes the developer-specific backend path, discovers installed SDK/AVD state, exports the discovered SDK to Gradle, and adds isolated launcher contracts. Mobile `12f949a8` makes refund refreshes lifecycle-owned and latest-wins, invalidates stale results on destruction, and guarantees mutation-loader cleanup. The launcher syntax/contract gates, real AVD discovery, `./gradlew help`, and all five focused refund tests passed. The exact APK installed from a checkout without `local.properties` and cold-launched to Login without a fatal exception, ANR, or OOM; the authenticated refund screen was unavailable on the signed-out install. |
| **Completed — other severity, canonical sport batch (1)** | `DATA-031` | Site `5aaa3101` introduces deterministic canonical sport identity, defensively deduplicates server/API/browser-cache reads, preserves and rewrites legacy references, merges compatible configuration, rejects conflicts, and enforces trimmed nonblank case-insensitive names in migration `20260713230000_enforce_canonical_sport_names`. Four focused suites passed 18/18 tests together with TypeScript, targeted lint, Prisma 7.8.0 checks, the loopback merge/conflict fixture, and a clean replay of all 153 migrations. Exact-commit browser validation rendered 15 sport filters with 15 unique names; selecting Indoor Volleyball changed only that named sport control, and the page produced no console warnings or errors. No live migration was deployed. |
| **Completed — other severity, image/loading batch (2)** | `APP-045`, `APP-056` | Mobile `ef781c82` moves Android image materialization to interruptible IO, streams to the server-matched 10 MiB limit, normalizes cancellation, and supplies actionable oversize feedback. The same commit replaces the global last-writer-wins loader with terminal per-operation ownership, overlap accounting, preserved message/progress fallback, CAS-race protection, and lifecycle cleanup for retained payment owners; all production call sites and fakes use the owner contract. Android and iOS compiled and all 69 focused tests passed, including six bounded-read/cancellation cases, seven loader ownership/race cases, and five overlapping player-action cases. The exact commit installed and cold-launched to Login without a fatal exception, ANR, or OOM. |
| **Completed — other severity, image-cleanup batch (1)** | `APP-068` | Mobile `6411b3c6` compensates a failed profile-image association by deleting the exact newly uploaded server file under a non-cancellable cleanup context, preserves the original association failure, and reports cleanup failure without masking it. All five focused repository/helper regressions passed, including the real upload-to-DELETE HTTP wiring and canceled-parent path, and the shared implementation compiled for the iOS simulator. The authenticated profile upload surface was not reachable on the signed-out emulator. |
| **Completed — other severity, event-filter batch (1)** | `APP-064` | Mobile `9c0f721b` makes normalized start-date selections durably active, exposes active/inactive filter semantics, keeps date ranges ordered, and treats the selected end date as the full local day across DST. Invalid price drafts now show an accessible error, disable Apply, retain the last valid applied range, and reset from external filter state instead of silently disagreeing. All 15 focused state/UI regressions passed and the shared implementation compiled for the iOS simulator. |
| **Completed — other severity, invite-search batch (1)** | `APP-082` | Mobile `60993db3` routes team and both chat invite searches through one debounced, generation-guarded coordinator. Every query synchronously clears prior rows, cancellation-ignoring late successes and failures are discarded, and clear/dismiss/back/destruction paths invalidate pending work. All 13 focused coordinator and dialog regressions passed, including pending-query stale rows and out-of-order failure coverage, and the shared implementation compiled for the iOS simulator. |
| **Completed — other severity, local-backend batch (1)** | `APP-025` | Mobile `61e6fc56` makes the one-command backend bootstrap authoritative on Bash and PowerShell: it rebuilds dependencies immutably from the sole lockfile, starts and waits for the effective Compose database, derives and pins both database variables to that loopback database, deploys tracked migrations, and accepts only the database-backed app-version contract as readiness. A shared atomic owner lock, process identity/tree checks, requested-and-recorded-port checks, and stale-owner fail-closed behavior prevent concurrent bootstraps or unsafe listener replacement. Shell and Node syntax, the local freshness/readiness/ownership contract, the repeated lock-race contract, the portable launcher contract, the Windows source contract, line-ending preservation, and scoped diff checks passed. Native PowerShell and real Docker execution were unavailable or intentionally excluded from this source/fake-runtime batch. |
| **Completed — other severity, cross-client state batch (6)** | `APP-011`, `APP-097`, `APP-099`, `APP-111`, `APP-113`, `AUD-007` | Site `74270f24` adds deterministic organization-review cursors, actionable invite paging/retention, supporting indexes, and one consistent organization route slug. Mobile `40db73e5` adds review paging, exact rental-context propagation, bounded payment-plan hydration, paged invite convergence, and ACKed outbox retention. The site batch passed 82 focused tests, TypeScript, Prisma validation/generation, and diff checks; the mobile batch passed 175 focused unit tests, iOS compilation, a real Room instrumentation test, installation, and two cold launches with successful local auth/version probes and no crash, ANR, OOM, SQLite, or Room failure. Browser validation appended a 51-row review fixture from 20 to 40 to 51 without replacement or duplication, then removed the fixture. |
| **Completed — other severity, avatar/build/chat batch (3)** | `APP-006`, `APP-007`, `APP-114` | Site `5f9acbef` exposes a membership-authorized, authoritative per-chat unread/latest-message summary. Mobile `65f22029` renders initials locally without per-avatar fallback requests, `726bf963` makes no-config debug/test builds reproducible while release builds fail fast and provides validated secret provisioning, and `0e00a789` replaces the 30-second full-chat poll with one foreground sync plus targeted push invalidation. Complete/partial summary ownership prevents user-switch races and preserves Room unread fallbacks. The focused site route passed 6/6 tests and TypeScript; mobile lifecycle, push, summary-ownership, unread aggregation, avatar, chat-list, and chat-terms regressions passed together with iOS simulator compilation and the Google Services contract. An isolated no-config clone built and tested, while release failed at the intended guard. The exact no-config APK cold-launched to Login with expected unavailable-Firebase diagnostics but no fatal exception, ANR, or OOM; the configured build was then restored and cold-launched cleanly. Independent review found no blockers. Deploy the site summary response before releasing the dependent mobile build. |
| **Completed — other severity, release/scheduler/test batch (3)** | `OPS-001`, `DATA-011`, `TEST-012` | Site `07243b93` holds the affiliate scheduler advisory lock on one dedicated PostgreSQL connection, reconciles and uniquely constrains app-release identities (including null build numbers), and removes the five assertion-free default Playwright probes. Four focused suites passed 19/19 tests; a disposable PostgreSQL 16 replay merged four duplicate release rows into two canonical rows and rejected both numbered and null-build duplicates; Prisma validation/generation, targeted ESLint, TypeScript, and scoped diff checks passed. |
| **Completed — other severity, event/cache integrity batch (2)** | `DATA-009`, `DATA-012` | Mobile `8a904f63` makes event collection conversion atomic, derives paging from server metadata or raw response size, and fails closed on corrupt nonblank Room/watch queue JSON without overwriting persisted operations. The integrated 45-test DTO/converter/repository batch passed, Android production/test compilation and iOS simulator compilation passed, all watchOS Swift sources type-checked, and independent review found no blockers. The watchOS corruption path has no executable unit-test target, so its current evidence is fail-closed source review plus full source type-check. |
...
| **Completed — other severity, Room-first catalog batch (2)** | `DATA-008`, `APP-005` | Mobile `dc1d4a8c` consolidates organization contracts and persists account-scoped organization, product, review, and time-slot snapshots through Room schema v92 with guarded fallback and transactional viewer rotation. Independent review found no blocker. An exact detached run passed 137/137 JVM tests, 3/3 iOS migrations, 3/3 Android migration/rollback tests, Android assembly, byte-identical APK installation, and a clean cold launch without a crash, ANR, Room, or migration failure. |
| **Completed — other severity, upload-policy/chat cleanup batch (2)** | `AUD-006`, `LEG-009` | Site `71d2a1bf` publishes the server-owned, versioned image-upload capability and validates uploads through that same policy; mobile `4edbcf1a` fetches and enforces the capability before upload. The site file-route suite passed 16/16 tests, mobile upload-policy/compensation suites passed 10/10 tests, and the current shared implementation compiled for the iOS simulator. Mobile `6809d6ba` removes the unreachable alternate chat header/message UI and its definition-only test while retaining the active chat scroll policy; the focused active-chat batch passed 13/13 tests and the iOS simulator compile passed. |
| **Completed — other severity, legacy DTO cleanup batch (1)** | `LEG-002` | Mobile `792f06c4` removes the eight definition-only DTO families, the unused route declarations, their dead converters, and the unused/non-atomic event and bulk-field methods. A current-source scan and a direct v1.6.13 tag scan found no production consumers; the mobile cleanup contract now rejects every removed path and symbol. The originally reported v80→89 migrations were superseded by registered legacy-profile bridges that remain necessary for installed-database upgrades and are intentionally retained. All 118 focused DTO/repository tests, Android debug assembly, iOS simulator compilation, cleanup contract, and scoped diff checks passed; independent review found no blocker. |
| **Completed — other severity, canonical Room membership batch (1)** | `APP-009` | Mobile `63451562` makes Room junctions the sole persisted team, pending-invite, user-team, and chat-membership truth; exact joins replace JSON substring queries, uncaught transactions roll back rejected replacements, and schema 93 removes the four duplicate array columns while preserving memberships whose user profiles have not arrived. The integrated branch passed 127/127 focused model/repository tests, 4/4 iOS migration tests, 12/12 Android on-device migration/DAO tests, iOS simulator compilation, and a schema comparison proving only the three parent and three junction entities changed. Independent review found no blocker. |
| **Remaining / not yet reconciled (5)** | All other headings in this report | Do not infer completion from an old or partial implementation. Each item must receive a current-source review, a focused regression test where code changes, and browser/emulator evidence when reachable. |

Current strict count: **222 completed, 5 remaining or not yet reconciled, 227 total findings**. This count deliberately excludes any pre-existing change that has not yet been revalidated against the current audit scenario.
...
| AND-ENV-004 | Wear build/test/runtime availability | Temurin JDK 17 + 384x384 Wear OS Small Round API 36 | pass | The current complete `:wearApp:testDebugUnitTest` suite passed 29/29 tests, `:wearApp:compileDebugKotlin` and `:wearApp:assembleDebug` passed, and the exact `668ee373` APK installed and cold-launched on the round Wear OS emulator. |
| AND-FLOW-007 | Wear login/demo officiating surfaces | Wear OS Small Round API 36, exact `668ee373` debug APK | resolved in audited Android/Wear scope | UI-tree-derived interaction rendered the timer at `73:17`, opened the existing incident editor, reached a round-safe explicit delete confirmation, and confirmed the optimistic deletion back to the remaining incident list. Fresh logs contained no fatal exception, ANR, or OOM. Login-field accessibility is covered by `APP-033`; duplicate empty-list refresh, delete, and timer notation are covered by `APP-030`, `APP-031`, and `APP-034`. |
...
| AND-FLOW-008 | Rental-remediation build launch and authenticated reachability | Pixel 9 Pro XL API 35, `fb9702ac`, installed 1.6.14 (67) | cold launch pass; authenticated rental UI blocked | The exact audited APK installed and reached Login after a force-stop cold launch. UI-tree-derived input reached the backend, but the supplied account returned `Invalid credentials`, so the authenticated rental timetable could not be exercised without changing account state. Fresh app/crash logs contained no fatal exception, ANR, or OOM; timetable interaction remains directly covered by the 39-test rental batch, including four Compose semantics tests. |
| AND-FLOW-009 | Mobile input-control remediation | Pixel 9 Pro API 35, exact `ddb3f0e0` debug APK | pass for reachable surface | The exact commit assembled, installed, and cold-launched to Login. UI-tree-derived navigation opened email signup, where Birthday was enabled and clickable; a coordinate tap opened the date picker. Fresh logs contained no fatal exception, ANR, or OOM. Authenticated discount, dropdown, and event-tag editors were unavailable on the signed-out install and remain directly covered by eight focused logic/Compose regressions plus iOS simulator compilation. |
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `a2ba3569` tracks the Room schema history and changes `.gitignore` to retain the canonical `composeApp/schemas/com.razumly.mvp.core.db.MVPDatabaseService/*.json` files. The current branch tracks 37 snapshots including the current v91 schema, and the Room Gradle configuration writes to that same tracked directory. The attached-emulator migration suite passed all 9 cases using those versioned fixtures.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `a2ba3569` establishes the supported historical source set and registers the chronological Android migration graph, including legacy bridges where an old on-device schema reused a version number. `RoomMigrationPathTest.allSupportedHistoricalSchemas_migrateThroughChronologicalGraph_toCurrentSchema` passed on the attached emulator together with all eight data-preservation/DAO regressions (9 tests total), with no destructive fallback in the Android database builder.
...
- Fix status: **compatibility implementation complete; operational removal gate remains open**. Web commits `0e77dc28` and `4621725f` install the additive defaults/index and retire application array writes, while `07265770` removes the obsolete Prisma team delegate; the remaining mutating `cleanup:user-teamids` operation is also retired in the current DATA-007 hardening batch. Authorized response aliases are now computed from `Products.organizationId` and active `TeamRegistrations`/`TeamStaffAssignments`, direct derived-field patches are rejected, and the read-only strict audit classifies legacy discrepancies without mutating data. A clean 157-migration replay, focused web/mobile suites, TypeScript/iOS compilation, an optimized release build, and a Playwright runtime flow passed. The strict audit's 13 focused tests also prove that unclassified links or invalid classification entries exit `2`, while a fully reconciled or already-removed state exits `0`. The physical columns are intentionally still present: the preparation migration and compatibility build have not been proven deployed in this branch's evidence, the last local strict audit still had four unclassified live-team links, and the required strict live audit plus 24-hour observation window have not been recorded. Do not add or deploy the destructive removal migration until those gates pass; this finding remains open.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `dc1d4a8c` retires the obsolete organization DTO copies and client-owned relationship arrays, centralizes API mapping in the network layer, and makes organizations, products, reviews, and time slots use account-scoped Room-backed snapshots with exact ordering, pagination, authoritative-empty handling, mutation invalidation, and safe offline fallback. Viewer changes purge transactionally and late responses cannot repopulate another account's cache. The exact detached commit passed 137/137 repository/DTO tests, 3/3 iOS migration tests, 3/3 Android migration/rollback tests, Android assembly, byte-identical APK installation, and a clean cold launch with no crash, ANR, Room, or migration failure; independent review found no blocker.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `8a904f63` converts event collection pages atomically with row/ID/validation context, fails retryably before Room writes when any row is malformed, uses explicit server continuation metadata for host/organization pages, and uses server `hasMore` or raw response size for filtered searches. The integrated DTO and repository batch passed 43 tests, Android production/test compilation and iOS simulator compilation passed, and independent review found no blockers.
...
- Fix status: **completed in the audited site branch; production migration/deployment pending**. `07243b93` deterministically merges duplicate identity rows, preserves safety-sensitive flags and user-visible change history, constrains both numbered and null-build identities, and coalesces duplicate active rows during a rolling deployment. Four focused suites passed 19/19 tests; a disposable PostgreSQL 16 replay reduced four duplicates to two canonical rows and rejected both duplicate forms; Prisma validation/generation, targeted ESLint, TypeScript, and scoped diff checks passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `8a904f63` makes every nonblank Room JSON decoder fail closed instead of silently returning empty/null, while preserving intentional absent null/blank semantics. The watchOS operation store now preserves unreadable queued bytes and propagates read/write failure through refresh and scoring actions; existing Wear OS behavior was already fail closed. Two exhaustive converter tests passed, all watchOS Swift sources type-checked, and independent review found no blockers. The watchOS corruption path has no executable unit-test target, so its residual evidence is source review and complete source type-check.
...
- Fix status: **completed in the audited branches; production/release deployment pending**. Site `531ab0b5` refuses to issue login or MFA session tokens to unverified accounts, makes `requireSession` reject any previously issued unverified session, and keeps `/auth/me` fail closed. Mobile honors the verification response instead of entering authenticated startup. The auth, MFA, and permission regressions passed in the 65-test web reconciliation batch.
...
- Fix status: **completed in the audited site branch; production deployment pending**. `36e1afd6` caps each preview dimension at 2,048px, caps requested output at four million pixels, refuses sources above 10MB before or during streaming, and enforces the canonical file-read boundary before object retrieval. The focused file-route and file-access regressions passed in the 65-test web reconciliation batch.
...
- Fix status: **completed in the audited site branch; deployment pending**. `416b6343` accepts only credential-free HTTP(S) on standard ports, rejects private/reserved/loopback DNS answers, pins the request to a validated address, revalidates every redirect, applies an eight-second total deadline, requires a supported image MIME type, enforces declared and streamed five-MiB limits, and caps Sharp decoding at 25 million pixels. The focused suite passed 28/28 tests, TypeScript and targeted lint passed, and the production transport downloaded a 958-byte public PNG through pinned DNS/TLS.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `bfc6e542` namespaces drafts by the authenticated account, enforces a 24-hour maximum lifetime with any valid payment-hold expiry as the earlier bound, rejects malformed/future/unsupported payloads, and migrates legacy data only when ownership is safe. Successful logout/account deletion clears only the captured current-account namespace, while failed logout preserves it for retry and other-account drafts survive. The seven focused DataStore regressions passed and were revalidated inside the current 148-test mobile batch.
...
- Fix status: **completed in the audited site branch; deployment pending**. `27f08f00` includes route handlers in coverage, adds 60/50/59/60 global floors, adds a route aggregate gate of 64/52/63/65, and runs the clean coverage/typecheck workflow for pull requests and `main`. The clean run passed 445/445 suites and 2,834/2,834 tests, gated 283 API route files at 65.71% statements, 53.90% branches, 64.93% functions, and 66.72% lines, and passed `npx tsc --noEmit`.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `21f5d85f`, `8280ffd0`, and `728e63c3` add focused action-gate, operation-store, and schedule-reconciliation regressions around serialized mutations, offline storage, and stale overlay expiry. The current focused Wear batch passed all six selected tests with zero failures, errors, or skips.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `a2ba3569` resolves `MAPS_API_KEY` through `defaultConfig` for application and generated unit-test manifests. The current standard `:composeApp:testDebugUnitTest` batches executed rather than failing during manifest processing: 148 unique focused tests passed, including the exact 15-test `MatchRepositoryHttpTest` and four-test `BracketGraphValidatorTest` classes.
...
- Fix status: **completed in the audited branches; deployment pending**. `1c6badfa` covers payment-success/order-failure recovery without a false booking or second charge, while `0f2c5e42` and `fb9702ac` cover the server/mobile contract for early, overnight, multi-day, DST, recurring-bound, deterministic-slot, and timetable accessibility behavior. The focused site rental route passed 15/15 tests plus `npx tsc --noEmit`; the combined mobile rental batch passed 39/39 tests plus the iOS simulator compile.
...
- Fix status: **completed in the audited site branch; production deployment pending**. `c7e57d13` routes event, schedule-created-event, and organization creation notifications through the configured canonical request-origin policy while retaining loopback development support. Four focused suites passed 84/84 tests, including hostile `Host` and `X-Forwarded-Host` regressions for every affected creation path.
...
- Fix status: **completed in the audited site branch; production deployment pending**. `570c09e9` initializes the shared PostHog client from `instrumentation-client.ts` only when a project token is configured, uses the configured ingestion host, mounts identity synchronization globally, and leaves capture/identify/reset as explicit no-ops without configuration. The client and identity regressions passed in the 65-test web reconciliation batch.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `143346de` makes the match-detail fake apply the same `MatchUpdateDto` transformation as the production outbox before emitting and returning state. The existing final-set regression now observes `COMPLETE`, `FINAL`, `actualEnd`, and the completed segment through that shared implementation; the focused 57-test component suite passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `d776877f` replaces the stale generic-topic assertion with exact league, tournament, weekly-event, and ordinary-event routing/payload cases. The Compose dialog regressions verify the exact trimmed title/body, keep the dialog and inputs present after failure, and prove neither success nor dismissal fires on rejection; all 4 focused notification tests passed as part of the 25-test Android batch.
...
- Fix status: **completed 2026-07-14**. The existing rental-resource regression now covers selection-on, draft synchronization, selection-off, and removal of both facility-field and custom-slot references. `fa121d77` adds monotonic user/team search generations plus clear, reset, dialog-dismiss, selection, and email-invite invalidation; the focused `EventInviteCoordinatorTest` suite passed all 12 cases, including out-of-order and late-after-clear responses.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `24faca6a` makes a blank or whitespace-only event name invalidate the aggregate result and adds the missing regression. The current consolidated mobile batch passed the ten-test `EventDetailsValidationTest` class with zero failures.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `752082f3` preserves host-authored incident definitions when copying match-rule overrides and adds the missing custom code, label, kind, participant/team requirement, removal, and reset coverage. The current consolidated mobile batch passed the 11-test `EventDetailsMatchRulesTest` class with zero failures.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ebf50ab5` adds four canonical-membership regressions for active players, active staff, inactive rows, and legacy fallback. Rerunning that suite with the existing money-input and participant-management coordinator suites passed, covering the cents/display-dollar round trip, one-shot mutation guards, and deferred A/B response ordering cited by this finding.
...
- Fix status: **completed in the audited site branch; deployment pending**. `07243b93` removes all five assertion-free `debug-*.spec.ts` probes from the default suite and adds a guard that fails if such probes return. The focused guard passed as part of the 19-test site batch.
...
- Fix status: **completed in the audited branches; deployment pending**. Site `9bd0a7d2` exposes the authenticated, versioned canonical quote contract backed directly by `billingFees.ts`. Mobile `69db9bf2` removes its fee constants and inverse calculation, validates every response strictly, uses a debounced generation-safe latest-request-wins coordinator, exposes retryable failures, and accepts only confirmed server cents across team registration, event create/edit, and participant billing. The site route/canonical fee batch passed 19 tests and TypeScript; the current mobile batch passed 148 focused tests, the iOS simulator compile, KSP, Android installation, and a cold launch plus email-signup interaction without a fatal exception, ANR, or OOM. No authenticated session was retained, so the price editors themselves were not manually reachable.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `dacf3f37` snapshots the unread rows, waits for the production HTTP client to acknowledge the receipt, and only then writes the acknowledged `readByIds` values to Room. The focused `MessageRepositoryHttpTest` suite passed all three cases, including a server 500 that leaves the cached message unread.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `c55f06be` keeps segment score and winner updates optimistic but removes the phone-only match-winner inference, leaving normal score-derived match winners to the canonical server response. The focused 15-test `MatchRepositoryHttpTest` suite covers a first completed set and a points-style plurality conflict; neither fabricates a local match winner.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `143346de` routes new incident creates and deletes directly through the repository's transactional outbox projection, removes the component's unchecked follow-up saves, and preserves only the explicit checked save for removing a legacy unsent create. Failed enqueue regressions prove both create and delete roll back their optimistic presentation, keep the cached score/incident authoritative, and surface a local-storage error; the focused 57-test component and 15-test repository suites passed.
...
- Fix status: **completed in the audited branches; deployment pending**. `076a0e56` moves desired staff roles and invite reconciliation behind one locked server transaction with revision conflict handling and a canonical response. `9424946e` makes mobile submit that desired state once and consume the returned snapshot instead of deleting, creating, and finally saving staff through separate requests; the new route, service, repository, and coordinator regressions cover conflict and failure boundaries.
...
- Fix status: **completed in the audited web branch; production migration/deployment pending**. `5aaa3101` adds deterministic normalized-name selection and defensive deduplication at the default-sport, API, and browser-cache boundaries. Migration `20260713230000_enforce_canonical_sport_names` resolves exact IDs before display-name fallbacks, rewrites every known scalar/array reference, preserves compatible configuration, rejects conflicting values, removes duplicates, and enforces trimmed nonblank case-insensitive identity. Four focused suites passed 18/18 tests together with TypeScript, targeted lint, Prisma 7.8.0 preflight, the loopback merge/conflict fixture, and a clean 153-migration replay. Exact-commit guest Discover validation rendered 15 sport controls with 15 unique names; toggling Indoor Volleyball changed only that named sport control, and the browser reported no console warnings or errors. The live database was inspected read-only and the migration was not deployed.
...
- Fix status: **completed in the audited site branch; deployment pending**. `07243b93` acquires, holds, and releases the session advisory lock through one dedicated PostgreSQL client, closes the connection when the lock is unavailable, and guarantees same-session unlock/close on success or failure. The focused scheduler regressions cover contention, normal release, and work-loading failure; the complete four-suite batch passed 19/19 tests plus TypeScript and targeted ESLint.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `dc1d4a8c` adds schema-v92 Room entities and DAOs for organization/product/query snapshots, reviews, and time slots; repository reads now write Room before returning, supported transient failures can fall back only to valid account/projection-scoped snapshots, and time-slot mutations update or invalidate the local source before success is exposed. The exact detached commit passed 137/137 repository/DTO tests, 3/3 iOS migration tests, and 3/3 Android migration/transaction tests, then assembled, installed byte-identically, and cold-launched without a crash, ANR, Room, or migration failure. Independent review found no hard blocker.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `65f22029` replaces the server-generated initials PNG with a local initials/jersey-number fallback, avoids a network model for missing images or initials-placeholder URLs, preserves the local fallback through loading/error, hides it after image success (including transparent images), and exposes one stable outer content description. The focused resolver/visibility regressions passed, shared iOS code compiled, and independent review found no blockers.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `726bf963` applies Google Services only when the ignored config exists, keeps debug builds/tests available without it, and makes release builds fail immediately with setup guidance. A tracked provisioner accepts a protected file path or masked base64 CI secret, validates project/package/app/key/OAuth metadata before atomic owner-only output, and never prints the config. Isolated no-config debug build/test and guarded-release paths passed; provisioned debug/release processing, shell/contract checks, and independent review passed. The exact no-config APK reached Login without a fatal exception, ANR, or OOM, and the configured build was restored and cold-launched cleanly.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `63451562` moves all four membership arrays out of Room parent rows, reconstructs the existing domain properties from exact junction rows, removes the user-side foreign keys so pre-profile membership remains valid, and lets transactional relation failures escape so Room rolls back the parent and junction replacement together. Android and iOS migrations backfill schema 92 into schema 93 without changing unrelated entities. The integrated branch passed 127 focused model/repository tests, four iOS migration tests, twelve Android on-device migration/DAO tests, and iOS simulator compilation; independent review found no blocker.
...
- Fix status: **completed across the audited branches; deployment pending**. Site `74270f24` adds a deterministic `createdAt`/ID cursor, validated cursor input, `nextCursor`, and append-only web pagination. Mobile `40db73e5` carries the cursor through its DTO/repository/component flow and guards refresh/load-more state against overlap and stale completion. Focused site/mobile regressions passed, and browser validation loaded a temporary 51-review fixture in stable 20-row pages through all 51 unique rows before the fixture was removed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `fb209fc4` upgrades the Gradle 9-compatible Foojay resolver, commits cross-platform daemon criteria for Adoptium 17, and makes the emulator launcher reject a regressed daemon contract before building. Both shell contracts passed. With the wrapper launched from JBR 21, the Gradle and Kotlin compiler daemons selected Temurin 17; KSP passed, `:composeApp:installDebug` installed versionCode 67, and the freshly installed app reached Login after a clean force-stop/relaunch. Native Windows execution is unavailable on this Mac, but the committed Windows x64 toolchain URL and vendor/version contract are validated statically.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `03bef4a5` gives Chats and Event Management distinct loading, actionable failure, and valid-empty states. Chats can open creation from its empty state; Event Management can create from a true empty state and retains a guarded full-screen retry after initial-load failure instead of claiming the account has no events. The focused component and Compose regressions passed, including persistence after transient-popup dismissal, and independent re-review confirmed the error/empty distinction.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `eacd608d` preserves the two-stage map confirmation while presenting autocomplete results in an explicit “Choose a map pin” panel with distinct address-aware buttons, screen-reader descriptions, and standard button actions. Both focused Compose semantics regressions passed; the combined 14-test mobile batch and iOS simulator compile passed. The exact APK cold-launched and completed Login → email signup → Login without a fatal exception, ANR, or OOM, but the authenticated Create Event map was unavailable on the signed-out install.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `03bef4a5` explains that templates are created from an existing personal event, points users to the event action, and provides a direct Create an event CTA. Its focused Compose semantics regression passed and the shared implementation compiled for Android debug and the iOS simulator.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `4ce92ab7` separates `androidx.core:core-ktx` from `androidx.test:core`, keeps production Core KTX only in the repository implementation that uses it, scopes test core to Compose Android unit tests and Wear tests, and removes the unused core UI dependency. The dependency contract passes and rejects alias ambiguity, direct-coordinate bypasses, duplicate/out-of-scope test aliases, and unnecessary production declarations.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `49295c3c` makes local Maven resolution an explicit property/environment opt-in, removes the developer-specific WSL fallback, preserves `MVP_SITE_DIR`, discovers standard SDK locations and installed AVDs, honors explicit overrides, fails clearly when discovery is empty, and exports the discovered SDK for Gradle. Shell syntax, the isolated portable-launcher contract, the aggregate launcher/daemon contract, real read-only AVD discovery, `./gradlew help`, and `git diff --check` passed. A first clean-worktree install caught the missing SDK export; after the contract/fix, `:composeApp:installDebug` succeeded and the exact APK cold-launched to Login without a fatal exception, ANR, or OOM.
...
- Fix status: **completed in mobile `61e6fc56`; release deployment pending**. Bash and PowerShell launchers now perform an immutable lockfile install on every bootstrap, wait for the effective Compose Postgres service, bind installation/migration/server children to its derived loopback URL, deploy tracked migrations, and require the database-backed app-version response before declaring readiness. Cross-platform atomic locking and verified requested/recorded-port ownership prevent concurrent or stale listener replacement. The syntax, freshness/readiness/ownership, lock-race, portable-launcher, and Windows source contracts passed; native PowerShell and real Docker execution were not performed in this batch.
...
- Fix status: **completed 2026-07-13**. `ab279fd9` makes `dev.ps1` fail before artifact reuse when the backend times out, Gradle exits nonzero, `adb install` fails, or app launch fails; device discovery and dependency installation now also validate their native exit codes. The focused `bash scripts/tests/dev-launcher-contract.sh` regression passed, including the ordering assertion that the Gradle failure check precedes any existing-APK presence check. PowerShell is not installed in this macOS environment, so native Windows execution remains a documented environment limitation rather than untested claimed runtime evidence.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `668ee373` removes the empty-branch Refresh chip and retains the one persistent Refresh action beside Logout. The Compose regression asserts exactly one Refresh node in the empty state; the complete Wear suite passed 29/29 tests and independent review found no blocker.
...
- Fix status: **completed for the audited Android/Wear scope; release deployment pending**. `668ee373` adds an edit-only Delete action, a separate loading-gated confirmation route, and a repository method that validates the incident then submits the established `{ action: "DELETE", id }` operation through the existing optimistic cache and durable outbox path. Focused payload/confirmation regressions and the complete 29-test Wear suite passed. On the 384x384 round emulator, UI-tree-derived interaction reached the confirmation and returned to a list with the selected incident removed; a live server acknowledgment remains dependent on an authenticated official-match fixture. Apple watchOS platform-only code remains excluded from this audit's implementation scope.
...
- Fix status: **completed 2026-07-14**. `ebb342c6` adds persistent `Email` and `Password` accessibility descriptions to the editable fields and explicitly marks the transformed password field as secret. The focused real-Compose semantics regression passed. A freshly installed Wear APK was exercised on the available phone emulator as a constrained accessibility check: after UI-tree-derived entry, both labels remained present as distinct semantics nodes, the password field stayed marked as a password, and UI Automator exposed only masked password text. A Wear OS AVD is still unavailable, so round-screen layout evidence remains outside this finding's closure.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `668ee373` centralizes match-clock formatting as unpadded minutes with two-digit seconds across regulation and added-time paths. The focused regression covers `0:00`, `73:52`, and `100:04`; the complete Wear suite passed 29/29 tests. The exact APK rendered `73:17` on a 384x384 round Wear OS emulator with no crash, ANR, or OOM.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `de36f944` sends a terminal `PaymentResult.Failed` on Android and iOS for missing purchase intent, publishable key, client secret, platform presentation object, context, or native view factory, and catches platform presentation/configuration exceptions. Eight focused validation/failure tests passed; the iOS simulator compilation target passed. A real Stripe submission remains outside local validation because it requires a safe server-issued client secret.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ef781c82` makes photo conversion suspending and moves Android metadata/open/read work into cancellation-aware interruptible IO. The reader streams only through the server-matched 10 MiB boundary, rejects a declared oversize before opening the stream, preserves active provider failures, normalizes interrupted reads back to coroutine cancellation, and reports actionable event/profile oversize feedback. Android and iOS production/test code compiled; the 69-test focused batch passed, including all six off-thread, cancellation, provider-error, exact-limit, streamed-overflow, and declared-overflow regressions. The exact APK installed and cold-launched to Login without a fatal exception, ANR, or OOM; an authenticated image picker was unavailable on the signed-out install.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ddb3f0e0` gives fractional fields an explicit `decimal` contract on Android and iOS while preserving `number`, `numbers`, and `money` as integer keyboards; the discount-percent editor now requests the decimal contract. Four focused resolver/display tests passed, Android production code compiled through the Compose interaction suite and exact APK assembly, and the iOS simulator target compiled.
...
- Fix status: **completed 2026-07-13**. `d6599868` makes the Android map `InstanceKeeper` cleanup cancel the component-owned coroutine scope, stop tracking, and unregister the callback if it remains registered. `MapComponentCleanupTest` passed both lifecycle paths, including a location-stop failure, and verifies the job is cancelled and the back callback is removed in either case.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ddb3f0e0` moves display resolution into one shared helper used by Android and iOS and falls back to each authoritative raw value when its option is not loaded. Focused pure and Compose regressions preserve `Loaded label, legacy-id` for a mixed loaded/unloaded selection; Android assembly and iOS simulator compilation passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ddb3f0e0` keeps interactive read-only fields enabled, exposes labeled Button semantics and current-value/error state, restores default clipped press indication, and uses a semantics-cleared overlay so real coordinate taps reach the action exactly once. The standalone Search date field follows the same contract. Four Compose interaction regressions passed semantic and coordinate activation, and the exact installed APK exposed the Birthday field as enabled/clickable and opened its date picker from a UI-tree-derived coordinate tap without a crash, ANR, or OOM.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ef781c82` issues one terminal owner token per logical operation, updates repeated stages in-place, retains every overlapping owner's message/progress, and keeps the oldest active owner visible until it finishes. An internal registry revision closes concurrent hide-versus-show/progress resurrection, and lifecycle destruction clears retained Profile, Organization, and Event Detail payment owners. Every production caller and focused fake now uses the owner contract. Android and iOS compiled and all 69 focused tests passed, including seven handler ownership/race/cleanup tests and five Player Interaction cases proving the first overlapping completion cannot hide the second. The exact APK installed and cold-launched to Login without a fatal exception, ANR, or OOM; authenticated overlapping actions were unavailable on the signed-out install.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ddb3f0e0` suppresses focus-driven reopening after selection, clears focus, and provides separate non-consuming pointer and FocusRequester-backed accessibility activation paths. The regression performs two independent collapse cycles, reopening once through semantic activation and once through a physical coordinate tap; disabled fields do not expose the custom click action. The four-test Compose suite, Android exact-commit assembly/install, and iOS simulator compilation passed, and independent review found no blocker.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `caec47c1` gives address resolution a monotonic generation, cancels and invalidates pending work before forwarding manual edits, and applies success, failure, and loading teardown only for the latest selection. Non-cancellable continuation regressions prove a manual edit and an out-of-order newer selection cannot be overwritten. All six focused tests and the combined 14-test mobile batch passed; iOS compiled and the exact APK completed the signed-out smoke interaction without a fatal exception, ANR, or OOM. Authenticated billing entry was not reachable on the fresh install.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `9c0f721b` derives active state from the durable normalized start boundary, exposes active/inactive accessibility semantics, normalizes selected dates to inclusive ordered local-day bounds across DST, and makes invalid price drafts explicit and non-applicable without losing the last valid filter. The 15 focused state/UI regressions passed and the shared implementation compiled for the iOS simulator.
...
- Fix status: **completed in the audited site and mobile branches; release deployment pending**. Mobile commit `732d67cc` requests the newest 100 messages first, retains the server pagination metadata, accumulates pages in Room, and exposes guarded/retryable earlier-history loading without treating prepends as new messages. Site commit `24cd188c` adds a stable ID tie-breaker to indexed timestamp ordering. All 21 focused Android chat tests and all 6 focused web route tests passed, along with `npx tsc --noEmit`.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `03bef4a5` stores guide completion under an account-derived key, ignores/removes ambiguous device-global history, resets loaded/completed state on logout or account change, cancels an active account-A guide before account B can render, and rejects stale account-A completion snapshots. Five focused persistence/controller regressions passed, including A-to-B active-guide cancellation; independent re-review found no remaining account-boundary blocker.
...
- Fix status: **completed 2026-07-13; release deployment pending**. Mobile commit `6411b3c6` deletes the exact newly uploaded file when profile association fails, runs compensation despite caller cancellation, returns the original association failure, and logs plus suppresses cleanup failure without masking the primary cause. All five focused tests passed, including a real MockEngine upload/DELETE wiring regression and canceled-parent cleanup; the shared implementation also compiled for the iOS simulator. The authenticated profile upload surface was unavailable on the signed-out emulator.
...
- Fix status: **completed 2026-07-13; deployment pending**. Web commit `e98f676a` adds a nullable sorted participant pair with a database uniqueness constraint, transactionally keys only one safe legacy winner per pair without rewriting older histories, uses an atomic server upsert, and preserves or clears the marker through membership, topic, archive, and restore lifecycle paths. Mobile commit `537245bf` consumes the canonical server row once per collector without a Room-triggered POST loop; the web provider deduplicates that returned row. Ten focused web suites passed 58 tests with TypeScript and Prisma checks, the clean 152-migration replay preserved legacy references, the concurrent pair exercise left one row, and the focused Android direct-message regression passed inside the 26-test aggregate. The authenticated browser flow was unreachable because the retained local session was signed out.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `d4a4e475` removes the edited lane, builds the known directed graph, and excludes a candidate only when the new source-to-candidate edge closes a path back to the source. It still tolerates unrelated legacy reference errors and preserves existing capacity/opposite-lane checks. All six focused graph tests and the combined 14-test mobile batch passed; iOS compiled and the exact APK completed the signed-out smoke interaction without a fatal exception, ANR, or OOM. An authenticated bracket editor was not reachable on the fresh install.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `12f949a8` binds the component scope to Essenty lifecycle destruction, serializes refreshes through a cancellation-safe latest-generation coordinator, invalidates all pending results when destroyed, and wraps approve/reject loader cleanup in `finally` without requiring prior handler injection. All five focused component tests passed with no refund-file warnings. The later exact APK containing this fix installed and cold-launched without a fatal exception, ANR, or OOM; the authenticated refund manager remained unreachable on the signed-out install.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `8a045707` replaces the one-shot 200-row read with 50-row server pages, persists each page into Room, renders only the IDs accepted for the active host/generation, deduplicates overlaps, rejects non-progressing cursors, and retries a failed load-more request from the same offset. The focused two-test component suite and one repository HTTP regression passed inside the 26-test consolidated Android batch; the freshly built and installed APK cold-launched normally.
...
- Fix status: **completed 2026-07-13**. `f53356c9` retains the existing `SupervisorJob` isolation while binding the Team Management scope to the Decompose lifecycle, so all eager flows and launched work cancel on destruction. Its lifecycle cleanup disables and unregisters the maximum-priority editor callback. The focused two-test `TeamManagementLifecycleTest` proves both scope cancellation and callback removal.
...
- Fix status: **completed in mobile `60993db3`; release deployment pending**. Team and both chat invite paths now share a debounced latest-query coordinator that clears prior suggestions immediately and rejects cancellation-ignoring stale successes or failures by generation. All 13 focused coordinator/dialog tests passed, including the pending-query mismatch and out-of-order failure regressions, and the shared code compiled for the iOS simulator.
...
- Fix status: **completed across the audited web and mobile branches; deployment pending**. Web commits `ba4306c9` and `6ff66cfe` expose an authoritative, permission-aware field/date-window availability snapshot, clip opaque conflicts to the requested range, exclude unpriced slots, and preserve explicit zero-price rentals. Mobile commit `bc575940` replaces the 300-event reconstruction with that endpoint, validates the exact response window and field references, avoids overwriting complete Room fields with the partial projection, cancels stale requests, blocks interaction when coverage is stale, and revalidates every selection against current conflicts. DST wall-clock conversion and padded cross-time-zone coverage are regression tested. The five-test web route suite, one DTO test, two repository tests, ten loader tests, and twelve organization-detail tests passed; the freshly installed APK launched with Room open and no crash or ANR. Authenticated rental interaction was unreachable because the retained browser and emulator sessions were signed out.
...
- Fix status: **completed 2026-07-14; deployment pending**. Server commit `0720f586` and mobile commit `c40797cd` carry the same exact per-selection field/start/end rows through authoritative pricing, atomic temporary holds, and final order creation. Sorted per-field advisory locks reject non-identical interval overlaps; retries reconcile obsolete holds only after every requested window validates; final scheduling checks each selection independently so occupied gap time does not reject a disjoint cart. The four focused web suites passed 56/56 tests plus `npx tsc --noEmit`; mobile passed 59/59 focused repository/component tests and the iOS simulator shared compile. The installed Android branch cold-launched to Login without a fatal exception, ANR, or OOM; authenticated timetable interaction was unreachable because no valid session was retained.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `fb9702ac` gives every timetable cell a field/date/time/state/price label, replaces the text delete affordance with a labeled button, and adds focusable keyboard/custom resize actions plus continuation semantics. The combined 39-test rental batch passed, including the Compose accessibility suite, and the iOS simulator compile succeeded.
...
- Fix status: **completed across the audited branches; deployment pending**. `fb9702ac` derives the mobile timeline from configured inventory, retains early-morning availability, and represents overnight/multi-day selections with explicit dates while failing closed across DST gaps and overlaps. `0f2c5e42` aligns canonical server validation for overnight recurrence, nullable recurring bounds, and deterministic covering-slot selection. The site rental route passed 15/15 tests plus `npx tsc --noEmit`; the combined mobile rental batch passed 39/39 tests plus the iOS simulator compile.
...
- Fix status: **completed across the audited branches; deployment pending**. Site `74270f24` adds an indexed deterministic `createdAt`/ID cursor contract and append-only web load-more behavior; mobile `40db73e5` pages the same contract with overlap and stale-result protection. The focused review suites passed, and the rendered browser flow preserved the original rows while progressing from 20 to 40 to all 51 unique fixture reviews; the temporary local fixture was removed afterward.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `40db73e5` carries the completed rental context through navigation, resolves the canonical order response, and accepts it only when its normalized field/start/end item set exactly matches the requested reservation before clearing the pending order. Missing or mismatched items fail closed instead of opening an unbound event draft. The focused billing and create-event regressions passed in the 170-test Compose batch, and the shared implementation compiled for the iOS simulator.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `03bef4a5` models signature synchronization as separate progress state, renders it as non-error progress inside text/web signing UI, blocks duplicate text-signature actions while synchronizing, and clears progress on every success, failure, timeout, dismissal, or flow reset. The focused signing integration regression proves normal waiting never enters `errorState` and successful clearance leaves no stale progress; Android debug and iOS simulator compilation passed.
...
- Fix status: **completed across the audited branches; deployment pending**. `54d3aced` adds bounded date-window cursor pagination and explicit completeness metadata to the canonical schedule contract; `154ad89e` hardens non-progressing/incomplete-page handling. `9424946e` makes mobile exhaust progressing pages, deduplicate IDs, and discard stale refresh generations instead of replacing My Schedule from one capped response.
...
- Fix status: **completed across the audited branches; deployment pending**. `35e05f55` adds deterministic offset pagination and `hasMore`/`nextOffset` metadata to billing history. `9424946e` makes Payment Plans exhaust every owner page, deduplicate bills, and surface non-progressing or over-limit pagination as an error rather than presenting a partial financial list as complete.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `40db73e5` deduplicates owners/events, hydrates independent bill/payment/event work with a strict concurrency bound, surfaces partial-result warnings, and serializes whole refreshes under one cancellation-aware mutex so stale or canceled generations cannot replace current state. The focused hydration/cancellation regressions passed in the 170-test Compose batch and the shared implementation compiled for the iOS simulator.
...
- Fix status: **completed across the audited branches; deployment pending**. Site `74270f24` defaults the user endpoint to actionable pending rows, provides opaque deterministic cursor paging plus explicit status/history access, canonicalizes legacy statuses, and performs a bounded authorization-scoped 90-day terminal-row cleanup that preserves pending team reconciliation. Mobile `40db73e5` exhausts progressing pages before replacing Room state and removes terminal or no-longer-authorized rows, including guardian-visible child-team rows. The focused site invite suites and five mobile invalidation regressions passed.
...
- Fix status: **completed across the audited branches; deployment pending**. Site `5f9acbef` adds a membership-authorized per-chat summary using the same unread/latest-message filters as the list endpoint. Mobile `0e00a789` removes the timer and duplicate startup sync, performs one full refresh per foreground lifecycle session, and treats pushes as independent message and targeted-summary invalidations. Serialized complete/partial summary state is user-owned, rejects late previous-user results, falls back to Room for chats absent from a partial snapshot, and still counts pushed chats not yet in Room. The focused API, lifecycle, push-failure, ownership, unread-aggregation, list, and terms regressions passed; Android and iOS compilation, installed cold launch, and independent review passed. The site response must deploy before the dependent mobile release.
...
- Fix status: **completed across the audited branches; deployment pending**. `54d3aced` introduces a bounded next-action schedule endpoint, `154ad89e` hardens its completeness rules, and `9424946e` makes Root refresh that compact contract only while lifecycle-active instead of repeatedly downloading the full schedule snapshot.
...
- Fix status: **completed 2026-07-14; deployment pending**. `fa121d77` gives user and team searches independent monotonic generations and applies loading, success, and error state only to the latest active request. Clearing either search and closing or completing the invite flow invalidates pending work, so a late response cannot restore stale suggestions. All 12 focused coordinator regressions passed, including out-of-order results and late completions after clear.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `ebf50ab5` routes Schedule matching through synchronized canonical active player registrations and staff assignments while retaining legacy arrays only through the compatibility normalizer. All four focused canonical-active, canonical-inactive, and legacy-fallback regressions passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. The input intentionally retains cents for lossless submission, but users see formatted currency rather than raw cents. The focused 3-test money-display suite passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `f64bf8e2` validates every configured slot before preparing a payload and blocks creation with its one-based slot number when a field, day, time, or valid end is missing. The suite now rejects missing-field, missing-time, missing-end, and end-before-start slots rather than silently dropping them; all 41 focused event-creation component tests passed.
...
- Fix status: **completed in the audited branch; production deployment pending**. `2fd43b7b` makes guest entry a provider-owned `startGuestSession` transition that clears mounted authenticated state and sets `isGuest` before either handler navigates. The provider, landing, login, and onboarding suites passed all 26 focused tests; `npx tsc --noEmit` passed. In a local browser, the landing-page CTA reached the onboarding choice screen directly rather than redirecting back to Login.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `6809d6ba` removes the unreachable alternate chat header/message implementation and its definition-only component test while retaining the active `ChatScrollPolicy` consumed by `ChatGroupScreen`. The focused 13-test active-chat batch and iOS simulator compilation passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `792f06c4` deletes the definition-only DTO family, route declarations, converters, `EventDao.upsertEventWithRelations`, and `IFieldRepository.createFields`. Both current source and the v1.6.13 tag confirm those symbols had no production consumer; the only test references were dead-converter assertions and fake overrides removed with their interfaces. The apparent v80→89 conflict is no longer current: explicit orphan migrations were replaced by registered legacy database profiles and a v90 continuity bridge, which remain required to upgrade installed databases and are covered by the migration-path suites. The cleanup contract now rejects every deleted path and symbol. Its source scan passed together with 41/41 active event DTO tests, 64/64 event repository tests, 13/13 user repository tests, Android debug assembly, iOS simulator compilation, and independent review.
...
- Runtime behavior: focused component/controller coverage, TypeScript, focused ESLint, the production build, and read-only desktop/mobile browser acceptance pass. The release browser run had no console errors or horizontal overflow.
- Fix status: **completed on the audited branch; production deployment pending**. `EventDetailSheet.tsx` is now 580 lines and imports no event, payment, or registration service, while `EventForm.tsx` is now 644 lines with React Hook Form retained as its sole persisted draft owner, no direct effects, and clean focused ESLint. Focused controllers own stale request rejection, registration phase exclusivity, signing, checkout, navigation, presentation state, resources, slots, division editing/commit/normalization, staffing, submission, create-form reference hydration, cross-field invariants, atomic configuration actions, catalog/context hydration, registration-question editing, section visibility/navigation, and narrow RHF field writers; render-only views own the public details, registration panels and their composition, main-content composition, join card, event hero, responsive sheet/inline content layout, the complete EventDetail overlay stack, manual-payment settings, staff/officials, division configuration/editor, and the complete EventForm section tree. All new interaction owners remain below 600 lines and their direct regressions pass. Both facades meet their approximately 600/700-line targets. The final 24-suite/211-test EventForm gate and nine-suite/39-test EventDetail gate passed with TypeScript, focused ESLint, diff checks, and a full production build that generated 122 static pages. Live-database-backed browser acceptance passed at 1440x900 and 390x844 on guest Discover and the inline public event page, including mobile join-card expansion and the guest Create event auth guard. Authenticated save/reload and registration mutations were deliberately not replayed against live data and remain covered by the focused regression suites.
...
- Fix status: **completed in the audited web and mobile branches; deployment pending**. Site `71d2a1bf` publishes the versioned server policy from the same validator used by upload POSTs, and mobile `4edbcf1a` resolves MIME types and extensions from that capability before upload. The 16-test file-route suite, 10 mobile upload-policy/compensation tests, and iOS simulator compilation passed.
...
- Fix status: **completed in the audited mobile branch; release deployment pending**. `40db73e5` invokes seven-day ACK compaction from the single synchronized outbox owner, preserves the global maximum-sequence sentinel needed for monotonic enqueue ordering, rethrows cancellation, and treats only ordinary retention failure as best effort. Unit regressions cover invocation, sequence continuity, cancellation, and cleanup failure; the real attached-emulator Room test exercised the generated deletion SQL and sentinel retention successfully.
...
- 2026-07-12: Revalidated APP-134 in the audited branch. The provider-owned guest transition updates mounted auth state before navigation; all 26 focused provider, landing, login, and onboarding tests and the TypeScript check passed. Local browser validation reached the onboarding choices directly from the landing CTA. Production deployment remains pending.
- 2026-07-12: Remediated APP-044 in the audited mobile branch. Android and iOS now report terminal payment-sheet setup/presentation failures instead of leaving checkout loading; all eight focused validation/failure tests passed and the iOS simulator compile target succeeded. A real Stripe submission requires a safe server-issued client secret and remains outside local validation. Mobile release deployment remains pending.
...
- 2026-07-13: Remediated APP-069 across web and mobile. A sorted nullable participant pair now enforces one server-owned direct conversation, the migration leaves ambiguous legacy duplicates and references intact, and both clients consume the canonical row without a check-then-create or Room feedback loop. Focused web/Android tests, Prisma checks, the full 152-migration replay, and the concurrent upsert exercise passed. Deployment remains pending.
- 2026-07-13: Remediated APP-077 in the audited mobile branch. Event Management now consumes progressing server pagination, deduplicates page IDs, preserves cached rows across retryable failures, and ignores stale host generations. Its three focused regressions passed in the 26-test Android aggregate. Mobile release deployment remains pending.
- 2026-07-13: Remediated APP-088 across web and mobile. Rental availability now comes from the authoritative field/date-window conflict snapshot rather than a capped organization-event page, and stale or uncovered snapshots cannot authorize a booking. Web, DTO, repository, loader, and component regressions passed; a fresh audited APK build installed and cold-launched without a crash or ANR. Deployment remains pending.
- 2026-07-13: Remediated APP-129 in the audited mobile branch. Schedule's “show only my matches” predicate now uses synchronized canonical active player/staff membership with legacy fallback isolated in the compatibility normalizer. All four focused membership regressions passed. Mobile release deployment remains pending.
- 2026-07-13: Reconciled TEST-011 in the audited mobile branch. The canonical-membership, currency-display, single-flight participant action, and deferred-response ordering suites passed together on current source. Mobile release deployment remains pending.
- 2026-07-13: Completed TEST-004, APP-095, and APP-096 across the audited branches. Payment/order failure, server/mobile slot-contract, early/overnight/multi-day/DST behavior, deterministic slot choice, and timetable accessibility are covered by 15 passing site route tests, 39 passing mobile tests, `npx tsc --noEmit`, and the iOS simulator compile. Deployment remains pending.
- 2026-07-14: Remediated DATA-031 in the audited web branch. `5aaa3101` establishes canonical case-insensitive sport identity across the server, API, cache, and database migration; 18 focused tests, TypeScript, targeted lint, Prisma preflight, the duplicate merge/conflict fixture, and a clean 153-migration replay passed. Exact-commit guest Discover validation rendered 15 unique sport controls, changed only Indoor Volleyball among named sports, and produced no console warnings or errors. The live migration remains pending separate authorization.
- 2026-07-14: Remediated APP-045 and APP-056 in the audited mobile branch. `ef781c82` makes Android image reads interruptible, bounded to the server's 10 MiB limit, and cancellation-safe; it also gives the global loader terminal operation ownership, overlap accounting, CAS-race protection, and lifecycle cleanup. Android/iOS compilation and all 69 focused tests passed. The exact commit installed and cold-launched to Login without a fatal exception, ANR, or OOM. Mobile release deployment remains pending.
- 2026-07-14: Remediated APP-011, APP-097, APP-099, APP-111, APP-113, and AUD-007 across the audited branches in site `74270f24` and mobile `40db73e5`. The batch passed 82 focused site tests, 175 focused mobile unit tests, TypeScript, Prisma checks, iOS compilation, a real Room instrumentation test, browser pagination through 51 temporary reviews, and installed-APK cold-launch/API checks with no fatal runtime error. The temporary browser fixture was removed; production and mobile release deployment remain pending.
