...
| Does your app use multi-factor authentication? | No | Website authenticator-app MFA implementation is in progress locally for Stripe connected-account creation and for users who opt into MFA, but BracketIQ production login does not currently enforce deployed app-level MFA for every user. Intuit Developer account MFA is separate and does not make the BracketIQ app MFA-enabled. |
...
- Keep the questionnaire answer for app-level MFA as `No` until website authenticator MFA in `docs/financial-feature-authenticator-mfa-plan.md` is migrated, deployed, and verified across password login, Google OAuth, optional setup, and Stripe connected-account creation blocking.
