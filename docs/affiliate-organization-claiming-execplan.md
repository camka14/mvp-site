...
- [ ] Add organization-match APIs and convert the creation wizard to suggest claim, ownership transfer, or dispute before allowing a new profile.
- [ ] Add the web claim wizard, ownership badges, unclaimed callouts, official review responses, and trust-aware sorting.
...
Planning, interaction mockups, the additive data foundation, and the server claim workflow are working locally. The Prisma client includes every ownership model; domain policy, classification, verification, claim state changes, claimant routes, protected administrator routes, claim-specific MFA, and administrator notification are tested; the audit/backfill is deterministic and idempotent; and affiliate club imports no longer overwrite profiles after they leave the unclaimed state. The local classification currently contains 87 first-party claimed organizations, 68 unclaimed affiliate organizations, and 110 affiliate organizations requiring review; 101 of the review rows are caused by owner IDs that are absent from the local auth snapshot. The production build includes the new route tree. No live migration or live backfill has run. The creation matcher and wizard, public claim UI and badges, administrator dashboard UI, official review responses, trust-aware ranking, mobile presentation, scheduled reminder/expiry processing, and live rollout remain outstanding.
