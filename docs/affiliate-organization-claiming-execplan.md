...
- [ ] Add the administrator Claims dashboard UI, claimant/current-owner decision emails, ownership-transfer reminder/expiry processing, and scheduled stale-claim cleanup. The protected administrator APIs are complete.
...
- Observation: The configured local database has affiliate organization rows whose recorded owner ID is absent from local `AuthUser`.
  Evidence: The implemented audit found 101 such affiliate organizations. It classifies them as `REVIEW_REQUIRED` instead of treating an unverifiable owner as either an internal placeholder or a legitimate legacy claimant.
...
- Decision: An affiliate organization whose recorded owner account cannot be found is `REVIEW_REQUIRED`.
  Rationale: A missing local or live account cannot be proven to be an internal placeholder or a legitimate external owner. Manual classification is safer than making the profile claimable or fabricating a legacy claim.
  Date/Author: 2026-07-29 / Codex
...
Planning, interaction mockups, the additive data foundation, the server claim workflow, match-first organization creation, the public claim wizard, and public ownership presentation are working locally. The Prisma client includes every ownership model; domain policy, classification, verification, claim state changes, claimant routes, protected administrator routes, claim-specific MFA, administrator notification, privacy-safe matching, signed match acknowledgements, concurrency locking, duplicate enforcement, and the two-step creation wizard are tested. The audit/backfill is deterministic and idempotent; affiliate club imports no longer overwrite profiles after they leave the unclaimed state; and website edits cannot silently retain trust from a replaced domain. The local classification currently contains 87 first-party claimed organizations, 68 unclaimed affiliate organizations, and 110 affiliate organizations requiring review; 101 of the review rows are caused by owner IDs that are absent from the local auth snapshot. Desktop and 390-pixel browser checks intercepted the real local unclaimed `503 Baseball` affiliate profile and the claimed `BracketIQ` profile with the expected claim, transfer, and dispute actions while keeping ownership badges in the card footer. A real affiliate event detail also confirmed the unclaimed badge and claim callout after browser testing exposed and corrected an event-detail hydration omission. The production build includes the claim and match route trees. No live migration or live backfill has run. The administrator dashboard UI, official review responses, trust-aware ranking, mobile presentation, scheduled reminder/expiry processing, and live rollout remain outstanding.
...
The dry-run report classifies each organization deterministically. An affiliate organization with an internal administrator owner and no external management staff becomes a candidate for `UNCLAIMED`. An affiliate organization with a non-admin owner becomes `CLAIMED` with method `LEGACY_OWNER` but receives no “Website verified” signal. An internal-owner organization with external managers, conflicting source domains, or missing provenance becomes `REVIEW_REQUIRED` and does not expose a claim action. First-party organizations remain `FIRST_PARTY` and `CLAIMED`.
...
Claimability is accepted when first-party organizations never expose a claim CTA; known internal-owner affiliate organizations with no external managers do; ambiguous or externally managed affiliate organizations become `REVIEW_REQUIRED`; and public payloads do not disclose the internal owner email or claim evidence.
...
The local migration and backfill evidence from 2026-07-29 is:
    ...
    AFFILIATE_IMPORTED / UNCLAIMED: 68
    AFFILIATE_IMPORTED / REVIEW_REQUIRED: 110
