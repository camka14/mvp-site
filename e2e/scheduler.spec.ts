...
const buildFutureLeagueWindow = () => {
  const start = new Date();
  start.setUTCDate(start.getUTCDate() + 7);
  start.setUTCHours(10, 0, 0, 0);

  const slotEnd = new Date(start);
  slotEnd.setUTCDate(slotEnd.getUTCDate() + 56);
...
const scheduleWithDocument = async (
...
test.describe("scheduler parity", () => {
  ...
  test("league scheduling without slots returns expected error", async ({
    hostApi,
  }) => {
    ...
    expect(payload.error).toBe(
      "Unable to schedule league because no valid time-slot windows are configured. Add or extend time slots to continue.",
    );
  });

  test("league same-day schedules extend end date", async ({ hostApi }) => {
    const window = buildFutureLeagueWindow();
    const eventDocument = buildLeagueDocument({
      ...
      timeSlots: [
        {
          $id: `${SEED_EVENTS.scheduler.leagueSameDay.id}_slot_1`,
    ...
    const payload = await response.json();
    const start = new Date(
      payload.event?.start ?? eventDocument.start,
    ).getTime();
