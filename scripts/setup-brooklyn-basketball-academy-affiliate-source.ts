/** Local-only setup for the Brooklyn Basketball Academy stored-intake club package. */
...
const main = async () => {
  ...
  try {
    ...
    const organization = { updatedAt: new Date(), name: 'Brooklyn Basketball Academy (BBA)', location: 'Brooklyn, NY', address: null, description: BBA_ORG_DESCRIPTION, logoId, ownerId: owner.id, website: BBA_HOME_URL, sports: ['Basketball'], status: 'UNLISTED', publicPageEnabled: false, publicWidgetsEnabled: false };
