 ...
 * mapping, and (with `--scrape`) the reviewed club candidate. The setup also
 * downloads the official OBC mark, normalizes it to an opaque square, and
 * assigns it to the public organization.
 */
...
// The generic CLUB importer created this deterministic unlisted row during the
// first local setup. Keep the repair narrow so a real organization is never
// removed by name alone.
const LEGACY_GENERATED_DUPLICATE_ORG_ID = 'affiliate_org_oregon_basketball_club_oregon_basketball_club';
const LOGO_FILE_ID = 'affiliate_file_oregon_basketball_club_logo';
const LOGO_FILE_NAME = 'oregon-basketball-club-logo-square.png';
const LOGO_BACKGROUND = '#062d50';
...
const sourceMetadata = {
  ...
  logoNote: 'Official transparent circular court mark is rendered on a full opaque navy canvas matching the source mark. No transparent corners or inset background remain.',
...
const normalizeLogo = async (input: Buffer): Promise<Buffer> => {
  const logo = await sharp(input, { animated: false })
    ...
    .resize({ width: 1024, height: 1024, fit: 'contain', withoutEnlargement: false })
    .png()
    .toBuffer();
...
const upsertLogo = async (ownerId: string) => {
  const response = await fetch(OBC_LOGO_SOURCE_URL, {
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Oregon Basketball Club logo: ${response.status} ${response.statusText}`);
  }
...
const removeLegacyGeneratedDuplicateOrganization = async () => {
  const duplicate = await (prisma as any).organizations.findUnique({
    where: { id: LEGACY_GENERATED_DUPLICATE_ORG_ID },
    select: {
      id: true,
      name: true,
      website: true,
  ...
  const isExpectedGeneratedDuplicate =
    duplicate.name === CLUB_NAME
    && duplicate.website === OBC_HOME_URL
    && duplicate.status === 'UNLISTED'
    && duplicate.publicPageEnabled === false;

  if (!isExpectedGeneratedDuplicate) {
    console.warn(`Preserved unexpected organization at ${LEGACY_GENERATED_DUPLICATE_ORG_ID}.`);
    return;
  }

  const [eventCount, facilityCount, teamCount, candidateCount, sourceCount] = await Promise.all([
    (prisma as any).events.count({ where: { organizationId: duplicate.id } }),
    (prisma as any).facilities.count({ where: { organizationId: duplicate.id } }),
    (prisma as any).canonicalTeams.count({ where: { organizationId: duplicate.id } }),
    (prisma as any).affiliateImportCandidates.count({ where: { publishedOrganizationId: duplicate.id } }),
    (prisma as any).affiliateScrapeSources.count({ where: { organizationId: duplicate.id } }),
  ]);

  if (eventCount + facilityCount + teamCount + candidateCount + sourceCount > 0) {
    console.warn(`Preserved generated OBC duplicate because it has dependencies: ${duplicate.id}`);
    return;
  }

  await (prisma as any).organizations.delete({ where: { id: duplicate.id } });
  console.log(`Removed unreferenced generated duplicate organization: ${duplicate.id}`);
};
...
const main = async () => {
  ...
  await removeLegacyGeneratedDuplicateOrganization();
