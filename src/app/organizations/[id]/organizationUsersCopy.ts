...
export const buildOrganizationUsersSubtitle = (organizationName?: string | null): string => {
  ...
  if (!normalizedName) {
    return "Participant users and registered teams from this organization's events, plus hosts and staff from rental events using its fields.";
  }
  return `Participant users and registered teams from ${normalizedName} events, plus hosts and staff from rental events using ${normalizedName} fields.`;
};
