...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getSearchPageForSegments('clubs', []);
  return metadataForPublicSearchPage(page, fallbackSearchTitle('clubs'));
}

export default async function FindClubsPage() {
  const page = await getSearchPageForSegments('clubs', []);
  if (!page) {
    notFound();
  }
