...
export const buildCodexAffiliateIngestionObjective = (
  ...
  return [
    ...
    'For source-only work, run focused Jest, targeted ESLint, disposable setup/scrape',
    'validation, and scoped diff checks. Never run full-project npx tsc --noEmit.',
    'Use full TypeScript only for explicitly authorized shared importer or app changes.',
    'Use exact current Sports.name values. Never infer a volleyball or soccer surface.',
    'Handle unsupported source sport labels as human-review evidence only; do not write a',
    'mapping package or candidates. Complete such jobs as HUMAN_REVIEW_REQUIRED with',
