...
![BracketIQ club organization overview showing the club profile, management tabs, and organization workspace](/blog/manage-sports-club/01-club-organization-overview.png)
...
Open the `Teams` tab for long-term club team records, then use the relevant league, tournament, tryout, clinic, or camp participant list when the club needs to confirm which teams are attached to a specific program.

![BracketIQ participant list for reviewing registered teams, roster counts, billing status, and team management actions](/blog/manage-sports-club/02-club-teams.png)
...
Open the `Customers` tab when the club needs relationship and billing context. For a specific league, tournament, tryout, clinic, or camp, also review the event participant list so staff can see the active teams or registrants tied to that program.

![BracketIQ participant review screen for checking registered teams, billing actions, and event follow-up](/blog/manage-sports-club/04-customers-parents-players.png)
