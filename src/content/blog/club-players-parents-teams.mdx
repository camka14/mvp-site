...
Open the club organization, then review the `Teams` tab for long-term club teams and the relevant event `Participants` tab for teams attached to a league, tournament, tryout, clinic, or camp. If the club organization is not set up yet, start with [How to Manage a Sports Club With BracketIQ](/guides/manage-sports-club), then come back to this workflow.

The team list is the club's working roster map. For event-based programs, the participant list shows which teams are registered, the sport, roster expectations, billing state, document status, and organizer actions.

![BracketIQ participant list showing registered teams, roster counts, billing state, documents, and organizer actions](/blog/club-players-parents-teams/01-club-teams-rosters.png)
...
Open the `Customers` tab when the club needs relationship history, then use the event participant list when staff need to act on a current team registration, bill, document, or roster issue.

![BracketIQ participant review screen showing registered teams, billing actions, document status, and organizer follow-up](/blog/club-players-parents-teams/02-customers-parents-billing.png)
