...
- [x] (2026-07-14 19:41Z) Milestone 4: introduced one registration workflow phase owner and moved event-detail views, dialogs, overlays, registration panels, and main-content composition behind typed view models and actions; the stable facade now meets its approximately 600-line target.
...
- [x] (2026-07-14 19:07Z) Milestone 4aj: moved QR, participant dropdown, inline-auth, free-agent, registration-question, payment-plan, signing, checkout, billing, payment, manual-proof, and hold-timer overlays into the 340-line render-only `EventDetailOverlays`. Two direct payment-state regressions and the seven-suite/35-test EventDetail facade gate pass; `EventDetailSheet.tsx` is now 991 lines, TypeScript and focused ESLint are clean, and diff checks pass.
- [x] (2026-07-14 19:34Z) Milestone 4ak: moved host schedule/QR actions plus child, team, and individual registration-panel composition into the 255-line render-only `EventDetailRegistrationPanels`. Two direct branch/action regressions and the eight-suite/37-test EventDetail facade gate pass; `EventDetailSheet.tsx` is now 882 lines, TypeScript and focused ESLint are clean, and diff checks pass.
- [x] (2026-07-14 19:41Z) Milestone 4al: moved hero, public overview/program, summary, participant, join-card, affiliate, weekly-session, and docking prop composition into the 239-line render-only `EventDetailMainContent`. Two direct state/interaction regressions and the nine-suite/39-test EventDetail facade gate pass; `EventDetailSheet.tsx` is now 580 lines, every new owner remains below 600 lines, TypeScript and focused ESLint are clean, and diff checks pass.
...
- [x] (2026-07-14 18:24Z) Milestone 5j: moved event-tag loading and keyed organization/resource hydration into the 68-line `useEventFormCatalogController`, leaving the EventForm facade with no direct effects. Three direct catalog/context regressions and the complete 17-suite/194-test serial EventForm gate pass; TypeScript is clean, focused ESLint has zero errors, and diff checks pass.
- [x] (2026-07-14 18:30Z) Milestone 5k: moved registration-question addition, prompt/required edits, removal, reindexing, and section expansion into the 61-line `useRegistrationQuestionEditorActions`. Two direct editor regressions and the complete 19-suite/198-test serial EventForm gate pass; TypeScript and complete-file focused ESLint are clean, and diff checks pass.
- [x] (2026-07-14 18:50Z) Milestone 5l: moved changed-field projection, nested league/tournament/playoff updates, normalized pending staff writes, and participant-state guards into the 127-line `useEventFormFieldWriters`, keeping React Hook Form as the only persisted draft owner. Four direct writer regressions and the complete 22-suite/206-test serial EventForm gate pass; `EventForm.tsx` is now 984 lines, TypeScript and focused ESLint are clean, and diff checks pass.
- [x] (2026-07-14 19:00Z) Milestone 6: split the two oversized existing EventForm controllers and move the remaining section composition into a render-only component.
...
- [x] (2026-07-14 18:28Z) Milestone 6c: moved the manual-payment warning, provider/link editor, removal/addition actions, and instructions field into the 105-line render-only `ManualPaymentSettingsSection`. Two direct interaction regressions and the complete 18-suite/196-test serial EventForm gate pass; TypeScript is clean, focused ESLint has zero errors, and diff checks pass.
- [x] (2026-07-14 18:41Z) Milestone 6d: moved division-mode, single-division defaults, division editor, playoff controls, validation actions, and standard/affiliate summaries into the 387-line render-only `EventFormDivisionSection`. Two direct branch/interaction regressions and the complete 20-suite/200-test serial EventForm gate pass; `EventForm.tsx` is now 1,184 lines, TypeScript and focused ESLint are clean, and diff checks pass.
- [x] (2026-07-14 18:46Z) Milestone 6e: moved staff/official rendering and its dependent-field mutations into the 172-line render-only `EventFormStaffSection`, keeping the split staff controller as the state owner. Two direct visibility/invariant regressions and the complete 21-suite/202-test serial EventForm gate pass; `EventForm.tsx` is now 1,076 lines, TypeScript and focused ESLint are clean, and diff checks pass.
- [x] (2026-07-14 19:00Z) Milestone 6f: moved the complete top-level section tree into the 468-line render-only `EventFormSections` and its visibility/navigation/question actions into the 117-line `useEventFormSectionsController`. Five direct composition/visibility regressions and the complete 24-suite/211-test serial EventForm gate pass; `EventForm.tsx` is now 644 lines, every new owner remains below 600 lines, TypeScript and focused ESLint are clean, and diff checks pass.
...
- [x] (2026-07-14 20:22Z) Milestone 7: the final 24-suite/211-test EventForm gate and nine-suite/39-test EventDetail gate pass with TypeScript, focused ESLint, diff checks, and the full production build, including Prisma validation/generation, Next compilation and type checking, and generation of 122 static pages. A live-database-backed release server returned 200 for the event API. Read-only browser acceptance passed at 1440x900 and 390x844 for guest Discover and the inline Eastside Timbers event page: responsive content, the mobile join-card expansion, and the guest Create event auth guard worked without horizontal overflow or browser console errors. Authenticated save/reload and registration mutations were deliberately not replayed against the live database and remain covered by the focused regression suites.
...
- Observation: final browser acceptance had to preserve the live database while still exercising the actual release build.
  Evidence: guest Discover and the public inline event page covered desktop/mobile composition, join-card behavior, API hydration, overflow, and console diagnostics without writes. The supported guest Create event entry correctly redirected to login; authenticated save, checkout, signing, and registration mutations stayed in the focused regression suites instead of creating or changing live records.
...
Planning, implementation, and acceptance are complete. Milestone 2 moved weekly-session, division registration/eligibility, payment-plan, organization, schedule, and public-display calculations into focused modules. Milestone 3 moved event hydration and inline authentication behind request-safe controllers. Milestone 4 now has one mutually exclusive registration-phase owner plus focused weekly selection, authentication, checkout, signing, registration-question, join-entry/finalization, registration-confirmation, participant actions/views and model, child registration and team participant presentation, sheet summary, public metadata/schedule modeling, inline public overview/program details, individual/team registration panels, a cohesive join-card view, join-card docking, selection synchronization, inactive cleanup, manual-payment owners, a typed division-registration model backed by stateless bill/sign-link commands, one render-only overlay stack, render-only registration-panel composition, and render-only main-content composition. `EventDetailSheet.tsx` now measures 580 lines, down from 7,219, has no direct effects, passes complete-file ESLint, no longer imports event, registration, payment, or bill services, and meets its approximately 600-line facade target. EventForm payment, lifecycle, resource, request-safe slot, submission, division commit/normalization, external reference hydration, cross-field invariant, atomic configuration-action, catalog/context, registration-question editor, and narrow RHF writer ownership are extracted; render-only views own manual-payment settings, staff/officials, division configuration/editor, and the complete section tree. `EventForm.tsx` now measures 644 lines, down from 4,376 at this plan's baseline, meeting its approximately 700-line facade target with no direct effects and clean complete-file focused ESLint. Both facade targets are met. The final focused gates, TypeScript, ESLint, diff checks, production build, and representative live-backed desktop/mobile browser acceptance pass; authenticated live-data mutations were intentionally left to the focused regression suites. Moving lines into equally broad hooks remains out of scope.
...
Revision note (2026-07-14): Completed Milestone 7 and AUD-003 after the final 24-suite/211-test EventForm gate, nine-suite/39-test EventDetail gate, TypeScript, focused ESLint, diff checks, production build, and read-only live-backed desktop/mobile browser acceptance passed without browser console errors or horizontal overflow.
