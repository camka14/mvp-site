...
Planning and current-source mapping are complete. Milestone 2 moved weekly-session, division registration/eligibility, payment-plan, organization, schedule, and public-display calculations into focused modules. Milestone 3 moved event hydration and inline authentication behind request-safe controllers. Milestone 4 now has one mutually exclusive registration phase plus focused authentication, checkout, signing, registration-question, join-entry/finalization, registration-confirmation, participant actions/views and model, child registration and team participant presentation, sheet summary, public metadata/schedule modeling, inline public overview/program details, individual/team registration panels, join-card docking, selection synchronization, inactive cleanup, manual-payment owners, and a typed division-registration model backed by stateless bill/sign-link commands. `EventDetailSheet.tsx` currently measures 1,599 lines, down from 7,219, has no direct effects, passes complete-file ESLint, and no longer imports event, registration, payment, or bill services; the join-card shell and overlay composition remain substantial. EventForm payment, lifecycle, resource, and request-safe slot ownership are now extracted and `EventForm.tsx` measures 2,759 lines, down from 4,376 at this plan's baseline. Completion requires both facades to meet the ownership and runtime acceptance criteria below; moving lines into equally broad hooks is not sufficient.
