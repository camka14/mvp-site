...
- [x] (2026-05-06) Fixed tournament pool-play scheduling so generated pool divisions inherit the time slots assigned to their bracket division during schedule builds.
- [x] (2026-05-06) Fixed tournament pool standings row filtering so the selected pool only shows teams assigned to that generated pool.
...
Revision note, 2026-05-06: Continued implementation in the main checkout on branch `codex/tournament-pool-play` after the temporary worktree was removed. The event schedule page now treats tournament pool play as standings-capable, uses bracket divisions for the first schedule filter, adds a Pool filter sourced from generated pool divisions, and loads standings for generated pool divisions. Verification passed with `npm test -- --runTestsByPath "src/app/events/[id]/schedule/__tests__/page.test.tsx" --runInBand` and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Corrected the tournament standings UI so the Standings tab uses bracket divisions as the parent selector and a required generated pool selector beside it. The pool selector defaults to the first generated pool for the selected bracket, and standings rows now load and locally calculate against the selected pool division rather than the bracket division. Verification passed again with `npm test -- --runTestsByPath "src/app/events/[id]/schedule/__tests__/page.test.tsx" --runInBand` and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Fixed a response-shaping bug where event detail responses could omit generated tournament pool rows from `divisionDetails`, leaving the Standings tab with no pool options to render. Tournament pool-play event responses now use generated non-playoff pool division rows as the visible division details, falling back to the stored event divisions only if generated pools are absent. Verification passed with the focused page suite, `eventPatchSanitizeRoutes.test.ts`, `templatePrivacyRoutes.test.ts`, and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Investigated the live "Test tournament pools" event through dbhub and found that it had no generated pool rows and no `PLAYOFF` bracket division rows; the existing save path was persisting tournament registration divisions only as `LEAGUE` rows. Updated the event form draft builder so pool-play tournament registration divisions are cloned into `playoffDivisionDetails`, updated edit-mode hydration so managers edit bracket divisions rather than generated pools, and preserved `poolCount`/`poolTeamCount` through the PATCH normalization path. Verification passed with the focused schedule page suite, `eventPatchSanitizeRoutes.test.ts`, `templatePrivacyRoutes.test.ts`, `tournamentPools.test.ts`, and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Fixed the scheduler failure where generated pool divisions had no compatible time slots when hosts configured slots on the visible bracket division. `scheduleEvent` now expands tournament pool-play time-slot division coverage from each bracket division to its generated pools before `EventBuilder` constructs the schedule. Verification passed with `npm test -- --runTestsByPath src/server/scheduler/__tests__/tournamentTimeSlots.test.ts --runInBand`, `npm test -- --runTestsByPath src/app/api/events/__tests__/scheduleRoutes.test.ts --runInBand`, and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Fixed the Standings tab row filter for tournament pool play. The site had already selected the generated pool division for standings data, but the local fallback row builder added every event team after building the filtered pool team set. It now seeds rows from the filtered team map only, while still allowing selected-pool matches to add their teams. Verification passed with the focused schedule page suite, the standings route suite, and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Fixed stale generated placeholder teams during tournament pool-play rebuilds. `scheduleEvent` now strips prior synthetic `Place Holder` teams before regenerating placeholders, and schedule persistence removes omitted generated placeholders on normal builds instead of only when rebuilding without placeholders. This prevents one pool from accumulating old placeholder teams across rebuilds.

Revision note, 2026-05-06: Corrected public division capacity display for tournament pool play. The shared division capacity helper now collapses generated pool rows into their mapped bracket registration division, using summed pool capacity and unioned pool/bracket team ids, so event details show bracket capacities rather than Pool A/B/C rows. Verification passed with `npm test -- --runTestsByPath src/lib/__tests__/divisionCapacity.test.ts --runInBand`, `npm test -- --runTestsByPath src/app/discover/components/__tests__/EventDetailSheetDetails.test.tsx --runInBand`, and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Corrected public tournament pool-play registration options. `EventDetailSheet` now builds signup/dropdown options from bracket registration divisions for tournament pool play, falling back to generated-pool mappings only to infer hidden bracket options, while league playoff events continue to expose regular league divisions and hide playoff rows. Verification passed with `npm test -- --runTestsByPath src/app/discover/components/__tests__/EventDetailSheetJoinPaymentPlanTeamJoin.test.tsx --runInBand --testNamePattern "tournament bracket divisions|league divisions"` and `npx tsc --noEmit --pretty false`.

Revision note, 2026-05-06: Corrected public event-card division labels for tournament pool play. `EventCard` now uses a shared display helper that presents bracket registration divisions instead of generated Pool A/B/C rows, while league playoff cards continue to present regular league divisions rather than playoff rows. Verification passed with `npm test -- --runTestsByPath src/lib/__tests__/eventDivisionDisplay.test.ts --runInBand` and `npx tsc --noEmit --pretty false`.
