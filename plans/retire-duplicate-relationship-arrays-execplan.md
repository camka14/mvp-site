...
Required-array initialization appears in user creation/reset paths including `src/app/api/auth/register/route.ts`, Google and Apple auth routes, `src/app/api/family/children/route.ts`, `src/server/inviteUsers.ts`, `src/server/publicGuestRegistration.ts`, `src/app/api/auth/account/route.ts`, and `prisma/seed.e2e.ts`. Product-array initialization appears in organization creation, `src/server/affiliateImports/service.ts`, `prisma/seed.e2e.ts`, affiliate setup scripts, and `scripts/sync-affiliate-organizations-to-live.ts`. `scripts/cleanup-user-teamids-canonical.ts` and its `cleanup:user-teamids` package command must be retired because they will be invalid after column removal.
